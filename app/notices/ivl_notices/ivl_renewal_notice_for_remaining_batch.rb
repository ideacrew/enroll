class IvlNotices::IvlRenewalNoticeForRemainingBatch < IvlNotice
  attr_accessor :family, :data, :address
  ...
  def append_data
    notice.individuals=data.collect do |datum|
        ...
        PdfTemplates::Individual.new({
          ...
          :incarcerated=> datum["incarcerated"].try(:upcase) == "FALSE" ? "No" : "Yes",
          :citizen_status=> citizen_status(datum["citizen_status"]),
          :residency_verified => datum["dc_resident"].try(:upcase) == "TRUE"  ? "District of Columbia Resident" : "Not a District of Columbia Resident"
        })
    end
  end
