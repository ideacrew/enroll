      ...
      if organization.present?
        ...
        if plan_year.present?
          ...
          plan_year.activate! if plan_year.may_activate?
           if ENV['py_state'] == "enrolled" && !plan_year.is_enrollment_valid?# for exception case like business requested only(roaster with no ce), updating plan year state to enrolled from renewing enrolled
              previous_state = plan_year.aasm_state
              plan_year.update_attributes(aasm_state:"enrolled")
              puts "plan year state changed to Enrolled"
