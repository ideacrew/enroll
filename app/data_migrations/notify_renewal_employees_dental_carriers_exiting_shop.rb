  ...
  def migrate
    begin
      ...
      hbx_ids.each do |hbx_id|
        ...
        enrollments = person.primary_family.active_household.hbx_enrollments.active.by_coverage_kind("dental").shop_market.select{ |enr| enr.plan.present?} if (person.present? && person.primary_family.present?)
