    ...
    SponsoredBenefits::Organizations::PlanDesignOrganization.collection.aggregate([
      ...
      begin
        ...
        if benefit_group.update_attributes(title: title)
          ...
          puts "Failure: Update Failed for benefit_group: #{record['benefit_group_id']} for PlanDesignOrganization: #{plan_design_organization(record['plan_design_proposal_id']).legal_name}"
        end
      rescue Exception => e
