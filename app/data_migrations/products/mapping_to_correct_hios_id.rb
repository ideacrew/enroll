  ...
  def migrate
    ...
    feins.each do |fein|
      begin
        ...
        product = application.benefit_sponsor_catalog.product_packages.last.products.where(hios_id: hios_id).first
        product_id = product.id

        if product.present?
          application.benefit_packages.first.health_sponsored_benefit.update_attributes!(reference_product_id: product_id)
        ...
        ces.each do |ce|
          enrs = ce.employee_role.person.primary_family.enrollments rescue nil
          if enrs.present?
