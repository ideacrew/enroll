...
class SendEmployeePlanSelectionConfirmationNotice < MongoidMigrationTask
  ...
  def trigger_plan_selection_confirmation_notice(census_employee)
    if !census_employee.employee_role.present?
      puts "can not send notice to ce with id #{census_employee.id} due to no employee role"
    elsif !census_employee.active_benefit_group_assignment.present?
      puts "can not send notice to ce with id #{census_employee.id} due to no active active_benefit_group_assignment"
    elsif census_employee.active_benefit_group_assignment.hbx_enrollment.nil?
      puts "can not send notice to ce with id #{census_employee.id} due to no enrollment purchased"
    else
      ...
      puts "sent plan selection confirmation notice to ce with id #{census_employee.id}"
    end
  end
end
