...
class RemoveDependentFromEeEnrollment < MongoidMigrationTask
  def migrate
    ...
    if hbx_enrollment.nil?
      puts "No hbx_enrollment found with the given id"
    else
      member=hbx_enrollment.hbx_enrollment_members.where(id:enrollment_memeber_id).first
      if member.nil?
        puts "No enrollment member found with the given id"
      else
        member.delete
        puts "delete the member with id #{enrollment_memeber_id} from hbx_enrollment #{enrollment_id}"
      end
    end
  end
