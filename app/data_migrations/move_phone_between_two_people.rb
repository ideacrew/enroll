...
class MovePhoneBetweenTwoPeople< MongoidMigrationTask
  def migrate
    ...
    begin
      ...
      from_person.phones.clear
      from_person.save!
      to_person.save!
      puts "transfer phones from hbx_id: #{from_hbx_id} to hbx_id: #{to_hbx_id}" unless Rails.env.test?
    rescue => e
      puts "#{e}"
    end
  end
