    ...
    if primary_person
      begin
        tax_households = primary_person.primary_family.active_household.tax_households.tax_household_with_year(applicable_year).select do |thh|
          ...
          ed.max_aptc.to_f == max_aptc.to_f && ed.csr_percent_as_integer == csr_percent
        end
        tax_households.each {|tax_household| tax_household.update_attributes(effective_ending_on: nil)} if tax_households.present?
      rescue => e
