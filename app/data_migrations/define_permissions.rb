...
#The convention for a privilege group 'x' is  'modify_x', or view 'view_x'
  ...
  def initial_hbx
    ...
    Permission.create(name: 'super_admin', modify_family: true, modify_employer: true, revert_application: true, list_enrollments: true,
                      send_broker_agency_message: true, approve_broker: true, approve_ga: true,
                      modify_admin_tabs: true, view_admin_tabs: true, view_config_tab_as_admin: true)
  ...
  def build_test_roles
    ...
    u7 = User.create( email: 'themanda.super_admin@dc.gov', password: 'P@55word', password_confirmation: 'P@55word', oim_id: "ex#{rand(5999999)+a}")
    ...
    p7 = Person.create(first_name: 'super_admin', last_name: "amanda#{rand(1000000)}", user: u7)
    ...
    SuperAdminRole.create!(person:p7, permission_id: Permission.super_admin.id, subrole: 'super_admin', hbx_profile_id: hbx_profile_id)
    u7.roles << "super_admin"
    p7.save!
    u7.save!
  end
  ...
  def super_admin_can_view_configuration_tabs
    Permission.find_or_create_by(name: 'super_admin', modify_family: true, modify_employer: true, revert_application: true, list_enrollments: true,
                                                  send_broker_agency_message: true, approve_broker: true, approve_ga: true,
                                                  modify_admin_tabs: true, view_admin_tabs: true, view_config_tab_as_admin: true)
    Permission.hbx_staff.update_attributes!(view_config_tab_as_admin: false)
    Permission.hbx_read_only.update_attributes!(view_config_tab_as_admin: false)
    Permission.hbx_csr_supervisor.update_attributes!(view_config_tab_as_admin: false)
    Permission.hbx_csr_tier2.update_attributes!(view_config_tab_as_admin: false)
    Permission.hbx_csr_tier1.update_attributes!(view_config_tab_as_admin: false)
  end

  def grant_super_admin_access
    users = User.where(:email.in => ["hannah.turner@dc.gov","sarah.bagge@dc.gov"])
    users.each do |user|
      SuperAdminRole.create!(person:user.person, permission_id: Permission.super_admin.id, hbx_profile_id: user.person.hbx_id)
    end
  end
end
