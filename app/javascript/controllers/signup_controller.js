...
export default class extends Controller {
  ...
  validateInput() {
    ...
    if (
      ...
      document.querySelector('.pwHelper')
    ) {
      ...
      if (!this.isEmail(email)) {
        Swal.fire({
          ...
          onOpen: () => {
            ...
            const removeElements = popup.querySelectorAll(
              // remove elements that are not needed in the popup causing accessibility issues
              '.swal2-image, .swal2-input, .swal2-file, .swal2-range, .swal2-select, .swal2-radio, .swal2-checkbox, .swal2-textarea'
            );
