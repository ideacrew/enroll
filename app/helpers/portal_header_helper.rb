module PortalHeaderHelper
  ...
  def portal_display_name(controller)
    if current_user.nil?
      ...
      if current_user.try(:has_broker_role?) || current_user.try(:has_broker_agency_staff_role?)
        link_to "#{image_tag 'icons/icon-expert.png'} &nbsp; I'm a Broker".html_safe, get_broker_profile_path, class: "portal"
      else
        link_to "#{image_tag 'icons/icon-individual.png'} &nbsp; I'm an #{controller == 'employer_profiles' ? 'Employer' : 'Employee'}".html_safe, main_app.family_account_path, class: "portal"
      end
