module PortalHeaderHelper
  ...
  def broker_header
    if enrollment_name.present?
    header_text = "<div style='display:inline-block white-space: nowrap;'>&nbsp; I'm a Broker<br/>&nbsp; #{enrollment_name.try(:titleize)}</div>"
    else
      header_text = "<div style='display:inline-block white-space: nowrap; position: relative; top: -5px;'>&nbsp; I'm a Broker<br/>&nbsp; #{enrollment_name.try(:titleize)}</div>"
    end
