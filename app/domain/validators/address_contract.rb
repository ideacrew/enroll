...
module Validators
  class AddressContract < Dry::Validation::Contract
    ...
    rule(:address_1) do
      if values[:address_1].to_s.empty?
        key.failure("#{values[:kind].capitalize} Addresses: address 1 can't be blank")
      end
    end

    rule(:city) do
      if values[:city].to_s.empty?
        key.failure("#{values[:kind].capitalize} Addresses: city can't be blank")
      end
    end

    rule(:state) do
      if values[:state].to_s.empty?
        key.failure("#{values[:kind].capitalize} Addresses: state can't be blank")
      end
    end
