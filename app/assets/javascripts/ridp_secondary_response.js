document.addEventListener("turbolinks:load", function() {
  ...
  if (/wait_for_secondary_response/.test(window.location.href)) {
    var i = 1;
    ...
    var repeater = setInterval(function () {
      if ( i < number_of_times) {
        $.ajax({
          type: "GET",
          data:{},
          url: window.location.href.replace(/wait_for_secondary_response/, "check_secondary_response_received"),
        ...
        if ( i > 9 ){
          window.location = window.location.href.replace(/wait_for_secondary_response/, "service_unavailable")
        }
      }
    }, interval_time);
  }
