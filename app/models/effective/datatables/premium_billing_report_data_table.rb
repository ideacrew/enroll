    ...
    class PremiumBillingReportDataTable < Effective::MongoidDatatable
      ...
      datatable do
        ...
        table_column :full_name,:label => 'Employee Profile',
          :proc => Proc.new { |row|
          hbx = @effective_datatable.hbx_cache[row.id]
          hbx.map do |value|
            content_tag(:span, class: 'name') do
              name_to_listing(value.employee_role.person)
            end +
            content_tag(:span, content_tag(:p, "DOB: #{format_date value.employee_role.person.dob}")) +
            content_tag(:span, content_tag(:p, "SSN: #{number_to_obscured_ssn value.employee_role.person.ssn}")) +
            content_tag(:span, "HIRED:  #{format_date value.employee_role.census_employee.hired_on}")
          end.join('').html_safe
        ...
        table_column :title ,:label => 'Benefit Package',
          ...
          hbx.map do |value|
            content_tag(:span, class: 'benefit-group') do
              value.benefit_group.title.try(:titleize)
            end
          end.join('').html_safe
          },:filter => false, :sortable => false
        ...
        table_column :coverage_kind,:label => 'Insurance Coverage',
          ...
          content_tag(:span) do
            hbx.map do |value|
              content_tag(:span, class: 'name') do
                mixed_case(value.coverage_kind)
              end +
              content_tag(:span) do
                " | # Dep(s) Covered: ".to_s + value.humanized_dependent_summary.to_s
              end +
              content_tag(:p, (value.plan.carrier_profile.legal_name.to_s + " -- " + value.plan.name.to_s))
            end.join('').html_safe
          end
          }, :filter => false, :sortable => false
        ...
        table_column :cost,:label => 'COST',
          ...
          hbx = @effective_datatable.hbx_cache[row.id]
          hbx.map do |value|
            content_tag(:span, "Employer Contribution: ".to_s + (number_to_currency value.total_employer_contribution.to_s)) +
            content_tag(:div) do
              "Employee Contribution:".to_s + (number_to_currency number_to_currency value.total_employee_cost)
            end  +
            content_tag(:p,  content_tag(:strong, "Total:") +  content_tag(:strong, value.total_premium))
          end.join('').html_safe
          }, :filter => false, :sortable => false
      ...
      def arrayize(collection)
        return collection if @already_ran_caching
          ...
          fam.households.each do |h|
            @hbx_cache[fam.id] = h.hbx_enrollments 
             # h.hbx_enrollments.each{|hbx| @hbx_cache[fam.id] =  hbx if @hbx_enrollment_ids.include?(hbx.id)}.compact
          end
        end
      super(collection)
      end
