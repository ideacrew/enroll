module Effective
  module Datatables
    class PremiumBillingReportDataTable < Effective::MongoidDatatable
      ...
      def collection
        @collection ||= Queries::PremiumBillingReportQuery.new(hbx_ids)
      end
      ...
      def arrayize(collection)
        ...
        collection.each do |fam|
          fam.households.each do |h|
            h.hbx_enrollments.each{|hbx| @hbx_cache[fam.id] =  hbx if @hbx_ids.include?(hbx.id)}.compact
          end
        end
        super(collection)
      end
      ...
      def hbx_ids
        ...
        @hbx_ids ||=  query.execute.nil? ? [] : query.execute.hbx_enrollments.collect{|h| h._id}
      end
