...
module Effective
  module Datatables
    class IdentityVerificationDataTable < Effective::MongoidDatatable
      datatable do
        table_column :name, :label => 'Name', :proc => proc { |row| link_to row.full_name, resume_enrollment_exchanges_agents_path(person_id: row.id), rel: 'noopener noreferrer' }, :filter => false, :sortable => false
        ...
        table_column :document_type, :label => 'Document Type', :proc => proc { |row| link_to document_type(row), document_uploaded_path(row), rel: 'noopener noreferrer'}, :filter => false, :sortable => false
