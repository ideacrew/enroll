  ...
  module Datatables
    class IdentityVerificationDataTable < Effective::MongoidDatatable
      datatable do
        ...
        table_column :document_type, :label => 'Document Type', :proc => Proc.new { |row| link_to document_type(row), resume_person_enrollment_exchanges_agents_path(person_id: row.id) }, :filter => false, :sortable => false
