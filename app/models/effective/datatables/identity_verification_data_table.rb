...
module Effective
  module Datatables
    class IdentityVerificationDataTable < Effective::MongoidDatatable
      ...
      datatable do
        table_column :full_name, :label => 'Name', :proc => proc { |row| link_to_with_noopener_noreferrer(h(row.full_name), resume_enrollment_exchanges_agents_path(person_id: row.id)) }, :filter => false, :sortable => true
