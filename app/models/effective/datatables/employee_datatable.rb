      ...
      datatable do
        ...
        table_column :actions, label: "", :width => '50px', :proc => Proc.new { |row|
          ...
          dropdown = [
              ...
              ['Retro - Terminate', main_app.confirm_effective_date_employers_employer_profile_census_employees_path(@employer_profile, census_employee_id: row.id, census_employee: row.id, type: 'retro_terminate', tab: 'employees'), pundit_allow(HbxProfile, :terminate_employee?) ? 'ajax' : 'disabled'],
