    ...
    class EmployerDatatable < Effective::MongoidDatatable
      datatable do
        ...
        table_column :legal_name, :proc => Proc.new { |row|
          link_to row.legal_name.titleize, employers_employer_profile_path(row.employer_profile, :tab=>'home')}, :sortable => false, :filter => false
        ...
        table_column :plan_year_state, :proc => Proc.new { |row|
          @latest_plan_year = row.employer_profile.try(:latest_plan_year)
          @latest_plan_year.try(:aasm_state).try(:titleize)}, :filter => false
        table_column :effective_date, :proc => Proc.new { |row|
          #row.employer_profile.try(:latest_plan_year).try(:start_on)
