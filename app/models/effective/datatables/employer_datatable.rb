      ...
      datatable do
        ...
        table_column :actions, :width => '50px', :proc => Proc.new { |row|
          dropdown = [
           ...
           ['View Username and Email', get_user_info_exchanges_hbx_profiles_path(people_id: Person.where({"employer_staff_roles.employer_profile_id" => row.employer_profile._id}).map(&:id), employers_action_id: "family_actions_#{row.id.to_s}"), 'ajax']
          ]
