  ...
  class FinancialAssistanceApplicationEligibilityResponse < ::Acapi::Subscription
    ...
    def call(event_name, e_start, e_end, msg_id, payload)
      ...
      if application.present?
        ...
        log("payload_http_status_code for #{application.id} is: #{payload_http_status_code}", {:severity=>'critical'})
        if application.success_status_codes?(payload_http_status_code.to_i)
          log("#{application.id} received a ED with success status code.", {:severity=>'critical'})
