    ...
    def hbx_enrollment_update(new_model_event)
      ...
      if HbxEnrollment::REGISTERED_EVENTS.include?(new_model_event.event_key)
        ...
        if new_model_event.event_key == :employee_coverage_termination
          if hbx_enrollment.is_shop? && (CensusEmployee::EMPLOYMENT_ACTIVE_STATES - CensusEmployee::PENDING_STATES).include?(hbx_enrollment.census_employee.aasm_state) && hbx_enrollment.sponsored_benefit_package.benefit_group.plan_year.active?
