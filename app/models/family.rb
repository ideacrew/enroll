 ...
 # after_destroy :remove_family_search_record
  ...
  class << self
    ...
    def expire_individual_market_enrollments
      ...
      @logger = Logger.new("#{Rails.root}/log/expire_individual_market_enrollments_#{Time.now.strftime('%Y_%m_%d')}.log") if families.present?
      @logger.info "Started expire_individual_market_enrollments process at #{Time.now.to_s}" if @logger
  ...
  def contingent_enrolled_family_members_due_dates
    ...
    contingent_enrolled_active_family_members.each do |family_member|
      family_member.person.consumer_role.verification_types.active.each do |v_type|
