...
class Family
 ...
 # after_destroy :remove_family_search_record
  ...
  def find_existing_relationships(matrix)
    id_map = {}
    family_members_id = family_members.where(is_active: true).map(&:person_id)
    family_members_id.each_with_index { |hmid, index| id_map.merge!(index => hmid ) }
    ...
    matrix.each_with_index do |x, xi|
      x.each_with_index do |y, yi|
        if (xi < yi) && !matrix[xi][yi].blank?
          existing_relationships << [id_map[xi], Person.find(id_map[xi]).person_relationships.where(family_id: self.id, predecessor_id: id_map[xi], successor_id: id_map[yi]).first.kind,  id_map[yi]]
        end
      end
    end
    existing_relationships
    ...
    matrix.each_with_index do |x, xi|
      x.each_with_index do |y, yi|
        if (xi < yi)
          ...
          existing_relationships << {:predecessor => id_map[xi], :relation => relation_kind,  :successor => id_map[yi]}
        end
      end
    end
    existing_relationships
