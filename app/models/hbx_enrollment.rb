...
class HbxEnrollment
  ...
  def cancel_previous(year)
    ...
    self.household.hbx_enrollments.ne(id: id).by_coverage_kind(self.coverage_kind).by_year(year).cancel_eligible.by_kind(self.kind).each do |hbx_enrollment|
      ...
      if (hbx_enrollment.enrollment_signature == self.enrollment_signature && hbx_enrollment.plan.carrier_profile_id == self.plan.try(:carrier_profile_id) && !hbx_enrollment.is_shop? && TimeKeeper.date_of_record < hbx_enrollment.effective_on) || (hbx_enrollment.is_shop? && TimeKeeper.date_of_record < hbx_enrollment.effective_on)

        if hbx_enrollment.may_cancel_coverage?
          hbx_enrollment.cancel_coverage!
          hbx_enrollment.update_current(terminated_on: p.effective_on)
        end
      elsif (hbx_enrollment.enrollment_signature == self.enrollment_signature && hbx_enrollment.plan.carrier_profile_id == self.plan.try(:carrier_profile_id) && !hbx_enrollment.is_shop? && TimeKeeper.date_of_record >= hbx_enrollment.effective_on) || (hbx_enrollment.is_shop? && TimeKeeper.date_of_record >= hbx_enrollment.effective_on)
