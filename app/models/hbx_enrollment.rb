...
class HbxEnrollment
  ...
  def decorated_elected_plans(coverage_kind, market = nil)
    ...
    if EnrollRegistry.feature_enabled?(:temporary_configuration_enable_multi_tax_household_feature)
      result = ::Operations::PremiumCredits::Find.new.call({ family: family, year: effective_on.year, kind: 'aptc_csr' })
      group_premium_credit = result.value! if result.success?
      elected_plans = benefit_coverage_period.elected_plans_by_enrollment_members(hbx_enrollment_members, coverage_kind, group_premium_credit, market)
    else
