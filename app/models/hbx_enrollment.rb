...
class HbxEnrollment
  ...
  ENROLLED_STATUSES   = %w[coverage_selected transmitted_to_carrier coverage_enrolled coverage_termination_pe unverified].freeze
  ...
  scope :verification_needed, ->{ where(:is_any_enrollment_member_outstanding => true).or({:terminated_on => nil }, {:terminated_on.gt => TimeKeeper.date_of_record}).order(created_at: :desc) }
