class BenefitCoveragePeriod
  ...
  def earliest_effective_date_max
    [earliest_effective_date, start_on].max
  end
  ...
  def elected_plans_by_enrollment_members(hbx_enrollment_members, coverage_kind)
    ...
    Plan.individual_plans(coverage_kind: coverage_kind, active_year: earliest_effective_date_max.year).by_plan_ids(elected_plan_ids).to_a
  end
