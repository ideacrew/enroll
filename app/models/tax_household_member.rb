class TaxHouseholdMember
  ...
  def strictly_one_pdc_only
    unless [is_ia_eligible, is_medicaid_chip_eligible, is_totally_ineligible, is_uqhp_eligible].count(true) == 1
      self.errors.add(:base, "only one of the all pdcs should be a true value")
    end
  end
