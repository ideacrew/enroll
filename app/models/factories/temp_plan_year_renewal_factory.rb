  ...
  class TempPlanYearRenewalFactory
    ...
    def validate_employer_profile
      if @employer_profile.plan_years.renewing.any?
        raise TempPlanYearRenewalFactoryError, "Employer #{@employer_profile.legal_name} already renewed"
      end

      unless PlanYear::PUBLISHED.include? @employer_profile.published_plan_year.aasm_state
        raise TempPlanYearRenewalFactoryError, "Renewals require an existing, published Plan Year"
      end

      unless @employer_profile.is_primary_office_local?
        raise TempPlanYearRenewalFactoryError, "Employer primary address must be located in #{Settings.aca.state_name}"
      end
    end
    ...
    def clone_benefit_group(active_group)
      ...
      if reference_plan_id.blank?
        raise TempPlanYearRenewalFactoryError, "Unable to find renewal for referenence plan: Id #{active_group.reference_plan.id} Year #{active_group.reference_plan.active_year} Hios #{active_group.reference_plan.hios_id}"
      end

      elected_plan_ids = reference_plan_ids(active_group)
      if elected_plan_ids.blank?
        raise TempPlanYearRenewalFactoryError, "Unable to find renewal for elected plans: #{active_group.elected_plan_ids}"
      end
      ...
      eligible_employees(active_group).each do |census_employee|
        if census_employee.active_benefit_group_assignment #&& census_employee.active_benefit_group_assignment.benefit_group_id == active_group.id
          ...
          unless census_employee.renewal_benefit_group_assignment.save
            raise TempPlanYearRenewalFactoryError, "For employer: #{@employer_profile.inspect}, unable to save census_employee: #{census_employee.inspect}"
          end
        end
      end
      true
