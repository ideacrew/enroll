...
module Subscribers
  ...
  class DetermineSlcspSubscriber
    ...
    subscribe(:on_determine_slcsp) do |delivery_info, _metadata, response|
      ...
      logger.debug "on_magi_medicaid_iap_benchmark_products: payload: #{payload}"
      result = ::Operations::Subscribers::ProcessRequests::DetermineSlcsp.new.call(payload)

      if result.success?
        logger.debug "on_magi_medicaid_iap_benchmark_products: success: #{ap result.success} acked"
      else
        errors = result.failure.errors.to_h
        logger.debug "on_magi_medicaid_iap_benchmark_products: acked (nacked) due to:#{errors}"
      end
      ack(delivery_info.delivery_tag)
    rescue StandardError, SystemStackError => e
      logger.debug "on_magi_medicaid_iap_benchmark_products: error: #{e} backtrace: #{e.backtrace}; acked (nacked)"
      ack(delivery_info.delivery_tag)
    end
  end
end
