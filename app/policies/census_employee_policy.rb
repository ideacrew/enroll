class CensusEmployeePolicy < ApplicationPolicy
  ...
  def update?
    ...
    can_change =
      if @user.has_role?(:employer_staff) && !@user.has_role?(:broker)
        @user.person.employer_staff_roles.map(&:benefit_sponsors_employer_profile_id).map(&:to_s).include? @record.benefit_sponsors_employer_profile_id.to_s
  ...
  def show?
    ...
    return true if (@user.has_role?(:employer_staff) || @user.has_employer_staff_role?) && @user.person.employer_staff_roles.map(&:benefit_sponsors_employer_profile_id).map(&:to_s).include?(@record.benefit_sponsors_employer_profile_id.to_s)
