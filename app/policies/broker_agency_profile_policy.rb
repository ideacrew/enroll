class BrokerAgencyProfilePolicy < ApplicationPolicy
  def access_to_broker_agency_profile?
  	...
  	if broker_role 
  	  return true if broker_role.broker_agency_profile_id == bap_id && broker_role.aasm_state == 'active'
    end
    staff_roles = user.person.broker_agency_staff_roles || []
    staff_roles.detect{|r| r.broker_agency_profile_id == bap_id && r.aasm_state == 'active'}
  end
end
