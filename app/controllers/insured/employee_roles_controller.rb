class Insured::EmployeeRolesController < ApplicationController
  ...
  def match
    ...
    match_person = @employee_candidate.match_person
    @person = @employee_candidate
    if @employee_candidate.valid?
      check_and_update_for_staff_role_person(match_person) if match_person.present? && match_person.has_active_employer_staff_role?
  ...
  def check_and_update_for_staff_role_person(person)
    ...
    if (person.ssn.blank? || person.gender.blank?)
      person.update_attributes(encrypted_ssn: CensusMember.encrypt_ssn(@employee_candidate.ssn), gender:@employee_candidate.gender)
    end
   end
end
