class Insured::FamilyMembersController < ApplicationController
  ...
  def update
    ...
    if @dependent.update_attributes(params.require(:dependent)) && update_vlp_documents(consumer_role, 'dependent', @dependent)
      ...
      consumer_role.update_attribute(:is_applying_coverage,  params[:dependent][:is_applying_coverage]) if consumer_role.present?&& params[:dependent][:is_applying_coverage].present?
      respond_to do |format|
        format.html { render 'show' }
        format.js { render 'show' }
      end
