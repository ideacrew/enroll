class Insured::GroupSelectionController < ApplicationController
  ...
  def new
    ...
    if ['individual', 'coverall'].include? @market_kind || (@person.try(:has_active_employee_role?) && @person.try(:is_consumer_role_active?)) || @person.try(:is_resident_role_active?) || @resident
