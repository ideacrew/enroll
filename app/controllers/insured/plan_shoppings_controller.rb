class Insured::PlanShoppingsController < ApplicationController
  ...
  def plan_selection_callback
    ...
    if selected_plan.present?
      redirect_to thankyou_insured_plan_shopping_path({plan_id: selected_plan.id.to_s, id: params[:id],coverage_kind: params[:coverage_kind], market_kind: params[:market_kind],change_plan: [:change_plan]})
    else
