class Insured::PlanShoppingsController < ApplicationController
  ...
  def plan_selection_callback
    enrollment_effective_on = HbxEnrollment.find(params[:id]).effective_on if params[:id].present?
    year = enrollment_effective_on.present? ? enrollment_effective_on.year : Settings.checkbook_services.current_year
    @selected_plan = BenefitMarkets::Products::Product.where(:"hios_id" => params[:hios_id], :"application_period.min" => Date.new(year, 1, 1)).first
