class HbxAdminController < ApplicationController
  ...
  def edit_aptc_csr
    ...
    @current_year = (params[:year] || TimeKeeper.date_of_record.year).to_i
    ...
    @max_aptc = @family.active_household.tax_households.tax_household_with_year(@current_year).last.try(:latest_eligibility_determination).try(:max_aptc) || 0
    @csr_percent_as_integer = @family.active_household.tax_households.tax_household_with_year(@current_year).last.try(:latest_eligibility_determination).try(:csr_percent_as_integer) || 0
    @year_options = @family.active_household.tax_households.map(&:effective_starting_on).map(&:year).uniq
