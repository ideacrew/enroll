class Exchanges::QlesController < ApplicationController
  ...
  def manage_qle
    ...
    if params[:manage_qle_action] == 'new_qle'
      redirect_to new_exchanges_qle_path(attrs) && return
    elsif params[:manage_qle_action] == 'modify_qle'
      @qle = QualifyingLifeEventKind.find(params[:id])
      redirect_to edit_exchanges_qle_path(@qle, attrs) && return
    elsif params[:manage_qle_action] == 'deactivate_qle'
      @qle = QualifyingLifeEventKind.find(params[:id])
      redirect_to deactivation_form_exchanges_qle_path(@qle, attrs) && return
    end
  end

  def question_flow 
    redirect_to question_flow_exchanges_qle(@qle, attrs) && return
  end
