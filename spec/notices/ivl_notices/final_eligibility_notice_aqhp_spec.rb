...
RSpec.describe IvlNotices::FinalEligibilityNoticeAqhp, :dbclean => :after_each do
  ...
  let(:plan) { FactoryGirl.create(:plan, :with_premium_tables, market: 'individual', metal_level: 'gold', csr_variant_id: '01', active_year: TimeKeeper.date_of_record.year + 1, hios_id: "11111111122302-01") }
  let!(:hbx_enrollment) {FactoryGirl.create(:hbx_enrollment, household: family.households.first, kind: "individual", plan: plan, aasm_state: "auto_renewing", effective_on: Date.new(TimeKeeper.date_of_record.year + 1,1,1))}
