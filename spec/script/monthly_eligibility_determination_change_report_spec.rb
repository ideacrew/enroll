...
describe 'monthly_eligibility_determination_change_report' do
  before do
  ...
  before :each do
    ...
    tax_household.update_attributes!(created_at: DateTime.now - 1.month)
    invoke_monthly_eligibility_change_script
    @file_content = CSV.read("#{Rails.root}/monthly_eligibility_report_#{TimeKeeper.date_of_record.strftime('%m_%d_%Y')}.csv")
  end

  it 'should add data to the file' do
    expect(@file_content.size).to be 2
  end
...
def invoke_monthly_eligibility_change_script
  eligibility_change_script = File.join(Rails.root, "script/monthly_eligibility_determination_change_report.rb")
