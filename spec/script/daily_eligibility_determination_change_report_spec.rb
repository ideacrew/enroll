...
describe 'daily_eligibility_determination_change_report' do
  ...
  before :each do
    ...
    @file_context = CSV.read("#{Rails.root}/daily_eligibility_report_#{TimeKeeper.date_of_record.strftime('%m_%d_%Y')}.csv")
  end
  ...
  it 'should match with the max aptc' do
    expect(@file_context[1][5]).to eq(eligibilty_determination.max_aptc.to_f.to_s)
  end

  it 'should match with the csr percent as integer' do
    expect(@file_context[1][7]).to eq(eligibilty_determination.csr_percent_as_integer.to_s)
  end
