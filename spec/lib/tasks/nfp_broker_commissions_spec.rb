require 'rails_helper'
Rake.application.rake_require "tasks/nfp_broker_commissions"
Rake::Task.define_task(:environment)

RSpec.describe 'upload commission statements to s3', :type => :task, dbclean: :after_each do
  let(:site)                      { create(:benefit_sponsors_site, :with_benefit_market, :as_hbx_profile, :cca) }
  let!(:broker_organization)      { FactoryGirl.build(:benefit_sponsors_organizations_general_organization, site: site)}
  let!(:broker_agency_profile1)   { FactoryGirl.create(:benefit_sponsors_organizations_broker_agency_profile, organization: broker_organization, market_kind: 'shop', legal_name: 'Legal Name1') }
  let!(:person1)                  { FactoryGirl.create(:person) }
  let!(:broker_role1)             { FactoryGirl.create(:broker_role, aasm_state: 'active', benefit_sponsors_broker_agency_profile_id: broker_agency_profile1.id, person: person1, npn: "48115294") }
  let(:sub_folder)                { "#{I18n.t("date.abbr_month_names")[TimeKeeper.date_of_record.month]}-#{TimeKeeper.date_of_record.year}" }

  context "upload commission-statements to S3" do
    before :each do
      FileUtils.mkdir "commission_statements"
      FileUtils.mkdir "commission_statements/#{sub_folder}"
      File.new("commission_statements/#{sub_folder}/48115294_1024_07102018_COMMISSION_1024-001_R.pdf", "w")
    end

    after :each do
      FileUtils.rm_rf(Dir["#{Rails.root}/commission_statements"])
    end

    it "should create douments/commission-statements for broker_agency_profile" do
      Rake::Task['nfp:commission_statements_upload'].invoke
      broker_agency_profile1.reload
      expect(broker_agency_profile1.commission_statements.count).to eq 1
    end

    it "cannot create commission-statements as cannot find broker_role with given npn" do
      broker_role1.update_attributes!(npn: "40115264")
      Rake::Task['nfp:commission_statements_upload'].invoke
      broker_agency_profile1.reload
      expect(broker_agency_profile1.commission_statements.count).to eq 0
    end
  end
end
