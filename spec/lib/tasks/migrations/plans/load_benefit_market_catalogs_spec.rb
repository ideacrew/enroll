...
describe "load_benefit_market_catalogs", dbclean: :after_each do
  ...
  let!(:site) {FactoryBot.create(:benefit_sponsors_site, :with_owner_exempt_organization, site_key: EnrollRegistry[:enroll_app].setting(:site_key).item)}
  let!(:bm) {FactoryBot.create(:benefit_markets_benefit_market, site: site, site_urn: EnrollRegistry[:enroll_app].setting(:site_key).item)}
