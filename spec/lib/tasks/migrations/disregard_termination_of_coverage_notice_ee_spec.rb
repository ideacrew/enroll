...
RSpec.describe 'The employees of MA new groups received a notice in their accounts that their coverage was terminated because no payment was received from their employer', :type => :task do
  ...
  let(:notice_date) { TimeKeeper::date_of_record.strftime("%m/%d/%Y") }
  ...
  context "create employee messages" do
    it 'should create secure inbox message for employee' do
      ...
      expect(message.body).to eq "Please disregard the notice that you received on #{notice_date} stating that your employer was not offering health coverage through the #{aca_state_name} #{site_short_name}. This notice was sent in error. We apologize for any inconvenience this may have caused." +
