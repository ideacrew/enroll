...
RSpec.describe 'Load Rating Regions Task', :type => :task do
  ...
  context "rate_reference:update_rating_regions" do
    ...
    it 'expect to read xlsx file' do
      ...
      expect(xlsx.sheet(0).row(2)).to match_array(["01001", "Hampden", "No", "Rating Area 1"])
      expect(xlsx.sheet(0).row(3)).to match_array(["01002", "Hampshire", "Yes", "Rating Area 1"])
      expect(xlsx.sheet(0).row(4)).to match_array(["01002", "Franklin", "Yes", "Rating Area 1"])
      expect(xlsx.sheet(0).row(699)).to match_array(["02791", "Bristol", "No", "Rating Area 6"])
    end
    
    it "should have the right data" do
      file_path = File.join(Rails.root,'lib', 'xls_templates', "SHOP_ZipCode_CY2017_FINAL.xlsx")
      xlsx = Roo::Spreadsheet.open(file_path)
      expect(xlsx.sheet(0).row(2).first.to_i).to eq RateReference.first.zip_code
      expect(xlsx.sheet(0).row(2)[1]).to eq RateReference.first.county
      expect(xlsx.sheet(0).row(2).last).to eq RateReference.first.rating_region
    end
