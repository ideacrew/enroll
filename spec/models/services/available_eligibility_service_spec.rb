...
RSpec.describe Services::AvailableEligibilityService, type: :model, :dbclean => :after_each do
  context 'initialize' do
    ...
    it { expect(described_class.new('enrollment_id')).to be_a Services::AvailableEligibilityService }

    it { expect{described_class.new}.to raise_error(ArgumentError) }
  end

  context 'available_eligibility' do
    it { expect(described_class.new('enrollment_id').respond_to?(:available_eligibility)).to eq true }

    it 'should raise error for bad enrollment id' do
      service_instance = described_class.new('enrollment_id')
