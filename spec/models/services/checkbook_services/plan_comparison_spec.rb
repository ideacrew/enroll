...
describe Services::CheckbookServices::PlanComparision, dbclean: :after_each do
  ...
  describe "when ivl and send_extra_fields_to_checkbook enabled", dbclean: :after_each do
    ...
    subject { ::Services::CheckbookServices::PlanComparision.new(shopping_enrollment,[product1, product2]) }
    ...
    it "should not include the current plan if it is not in the list of plans" do
      body = ::Services::CheckbookServices::PlanComparision.new(shopping_enrollment,[product2]).construct_body_ivl
      expect(body[:currentPlan]).to be_nil
    end
  end

  describe "when ivl and send_extra_fields_to_checkbook disabled", dbclean: :after_each do
    let!(:person) { FactoryBot.create(:person, :with_consumer_role) }
    let!(:family) { FactoryBot.create(:family, :with_primary_family_member, person: person)}
    let!(:household) { FactoryBot.create(:household, family: family) }
    let(:year) {TimeKeeper.date_of_record.year}
    ...
    it "ivl body should not include current start date" do
      body = subject.construct_body_ivl
      expect(body[:coverageStartDate]).to be_nil
    end
