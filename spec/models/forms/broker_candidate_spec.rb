...
describe Forms::BrokerCandidate, "validations" do
  ...
  context 'when broker with same NPN alredy exists' do
    ...
    let(:person) { FactoryGirl.create(:person, first_name: "steve", last_name: "smith", dob: "10/10/1974") }
  ...
  describe ".save" do
    ...
    context 'when duplicate user present in the system with same personal information' do
      ...
      it 'should raise an error' do
        subject.save
        expect(subject.errors.to_hash[:base]).to include("a person matching the provided personal information has already been claimed by another user.  Please contact HBX.")
      end
    end
    ...
    context 'when applicant is a broker' do
      ...
      it 'should save person with a broker role' do
        expect(Person.where(first_name: "firstname", last_name: "lastname", dob: "1993-06-03")).to be_empty
        subject.save
        person = Person.where(first_name: "firstname", last_name: "lastname", dob: "1993-06-03").first
        expect(person).to be_truthy
        expect(person.broker_role).not_to be_nil
    ...
    context 'when applicant is a staff member' do
      ...
      it 'should save person with broker staff role' do
        expect(Person.where(first_name: "firstname1", last_name: "lastname1", dob: "1993-06-03")).to be_empty
        subject.save
        person = Person.where(first_name: "firstname1", last_name: "lastname1", dob: "1993-06-03").first
        expect(person).to be_truthy
        expect(person.broker_role).to be_nil
