...
describe "#trigger employer_renewal_dental_carriers_exiting_notice", type: :model, dbclean: :after_all do
  let(:employer_profile10) { create(:employer_with_planyear)}
  let(:plan_year10) {FactoryGirl.create(:plan_year, employer_profile: employer_profile10, aasm_state: "draft")}
  let(:carrier_profile) { FactoryGirl.create(:carrier_profile, organization: organization) }
  let(:dental_plan) { FactoryGirl.create(:plan, carrier_profile_id: carrier_profile.id) }
  let(:benefit_group) { FactoryGirl.create(:benefit_group, title: "silver offerings 1", plan_year: plan_year10, plan_option_kind: 'single_carrier', dental_plan_option_kind: 'single_carrier', dental_reference_plan_id: dental_plan.id, elected_dental_plan_ids: ["abcd"])}
  let(:organization) {FactoryGirl.create(:organization, legal_name: "Delta Dental")}

  before :each do
    allow(plan_year10).to receive(:benefit_groups).and_return([benefit_group])
  end

  it "should trigger employer_renewal_dental_carriers_exiting_notice job in queue" do
    ActiveJob::Base.queue_adapter = :test
    ActiveJob::Base.queue_adapter.enqueued_jobs = []
    plan_year10.notify_renewal_employer_dental_carriers
    queued_job = ActiveJob::Base.queue_adapter.enqueued_jobs.find do |job_info|
      job_info[:job] == ShopNoticesNotifierJob
    end
    expect(queued_job[:args]).to eq [plan_year10.employer_profile.id.to_s, 'employer_renewal_dental_carriers_exiting_notice']
  end

  it "should trigger employer_renewal_dental_carriers_exiting_notice job in queue" do
    organization.update_attributes!(:legal_name => "MetLife")
    ActiveJob::Base.queue_adapter = :test
    ActiveJob::Base.queue_adapter.enqueued_jobs = []
    plan_year10.notify_renewal_employer_dental_carriers
    queued_job = ActiveJob::Base.queue_adapter.enqueued_jobs.find do |job_info|
  ...
  it "does a state transition succesfully without any errors" do
    expect(plan_year10).to receive(:notify_renewal_employer_dental_carriers)
    plan_year10.renew_plan_year!
  end
end