  ...
  describe 'user' do
    ...
    context 'when password & password confirmation' do
      ...
      it 'does not match' do
        ...
        expect(User.create(**params).errors[:password]).to eq ["must have at least 4 alphabetical characters", "is too short (minimum is 8 characters)", "is too short (minimum is 8 characters)"]
        expect(User.create(**params).errors[:password_confirmation]).to eq ["doesn't match Password", "doesn't match Password"]
      end
    end
