...
RSpec.describe InsuredEligibleForBenefitRule, :type => :model do
  context "#is_benefit_categories_satisfied?" do
    ...
    it "should return true" do
      ...
      rule = InsuredEligibleForBenefitRule.new(consumer_role, benefit_package, 'dental',enrollment_kind)
  ...
  context "is_cost_sharing_satisfied?" do
    ...
    it "should return true when csr_kind is blank" do
      rule = InsuredEligibleForBenefitRule.new(ConsumerRole.new, benefit_package,enrollment_kind)
    ...
    it "should return true when cost_sharing is blank" do
      ...
      rule = InsuredEligibleForBenefitRule.new(@consumer_role, benefit_package_with_current_date_start_on,enrollment_kind )
      expect(rule.is_cost_sharing_satisfied?).to eq true
    end
  ...
  context "is_residency_status_satisfied?" do
    ...
    it "return false when consumer_role is nil" do
      ...
      rule = InsuredEligibleForBenefitRule.new(nil, benefit_package,enrollment_kind)
  ...
  context "is_lawful_presence_status_satisfied?" do
    ...
    let(:rule) {InsuredEligibleForBenefitRule.new(role, benefit_package,enrollment_kind)}
    let(:person) {double}
    let(:enrollment_kind) {''}
