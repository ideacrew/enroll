...
RSpec.describe 'ModelEvents::InitialEmployerNoBinderPaymentReceived', :dbclean => :after_each do
  ...
  let(:date_mock_object) { PlanYear.calculate_open_enrollment_date(TimeKeeper.date_of_record.next_month.beginning_of_month)[:binder_payment_due_date].next_day }
  ...
  describe "ModelEvent" do
    it "should trigger model event" do
      expect_any_instance_of(Observers::NoticeObserver).to receive(:deliver).with(recipient: employer_profile, event_object: plan_year, notice_event: notice_event1).and_return(true)
      expect_any_instance_of(Observers::NoticeObserver).to receive(:deliver).with(recipient: employee_role, event_object: plan_year, notice_event: notice_event2).and_return(true)
  ...
  describe "NoticeTrigger" do
    context "whne binder payment is missed" do
      ...
      let(:model_event) { ModelEvents::ModelEvent.new(:initial_employer_no_binder_payment_received, plan_year, {}) }
  ...
  describe "NoticeBuilder employer" do
    ...
    let(:subject) { Notifier::NoticeKind.new(template: template, recipient: recipient) }
    let(:merge_model) { subject.construct_notice_object }
    let(:next_available_start_date) {PlanYear.calculate_start_on_dates.first.to_date}
