...
RSpec.describe 'ModelEvents::BrokerFired', dbclean: :around_each  do
  ...
  let(:model_instance)          { broker_agency_account }

  describe "when an employer successfully fires a broker" do
    ...
    context "NoticeTrigger" do
      ...
      let(:model_event) { ModelEvents::ModelEvent.new(:broker_fired, model_instance, {}) }
      ...
      it "should trigger notice event" do
        ...
        expect(subject.notifier).to receive(:notify) do |event_name, payload|
          expect(event_name).to eq "acapi.info.events.broker_agency.broker_agency_fired_confirmation_to_agency"
          expect(payload[:event_object_kind]).to eq 'EmployerProfile'
          expect(payload[:event_object_id]).to eq employer_profile.id.to_s
        end
