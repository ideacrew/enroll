...
describe 'ModelEvents::InitialEmployerFinalReminderToPublishPlanYear', dbclean: :around_each do
  ...
  let!(:model_instance) { FactoryGirl.create(:plan_year, employer_profile: employer, start_on: start_on, aasm_state: 'draft') }
  let!(:benefit_group)  { FactoryGirl.create(:benefit_group, plan_year: model_instance) }
  let!(:date_mock_object) { (start_on.prev_month + 2.days) }
  ...
  describe "NoticeTrigger" do
    context "when initial employer 2 days prior to dead line" do
      ...
      let(:model_event) { ModelEvents::ModelEvent.new(:initial_employer_final_reminder_to_publish_plan_year, PlanYear, {}) }
