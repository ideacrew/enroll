...
describe 'ModelEvents::RenewalOeEmployeeAutoRenewalNotification' do

  let(:model_event)  { "renewal_oe_employee_auto_renewal" }
  let(:notice_event) { "renewal_oe_employee_auto_renewal" }
  ...
  describe "ModelEvent" do
    context "when renewal oe employee auto renewal" do
      ...
      it "should trigger model event" do
        ...
        model_instance.observer_peers.keys.each do |observer|
          expect(observer).to receive(:census_employee_update) do |model_event|
            ...
            expect(model_event).to have_attributes(:event_key => :renewal_oe_employee_auto_renewal, :klass_instance => model_instance, :options => {:event_object => renewing_plan_year})
          end
        end
        factory.renew
      end
  ...
  describe "NoticeTrigger" do
    context "when renewal oe employee auto renewal" do
      ...
      let(:model_event) { ModelEvents::ModelEvent.new(:renewal_oe_employee_auto_renewal, model_instance, {event_object:renewing_plan_year}) }

      it "should trigger notice event" do
        expect(subject).to receive(:notify) do |event_name, payload|
          expect(event_name).to eq "acapi.info.events.employee.renewal_oe_employee_auto_renewal"
