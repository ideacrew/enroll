...
describe 'ModelEvents::IneligibleInitialApplicationSubmitted', dbclean: :around_each do
  ...
  before do
    allow(model_instance).to receive(:application_eligibility_warnings).and_return({:fte_count => "Full Time Equivalent must be 1-50"})
  end
  ...
  describe "NoticeBuilder" do
    ...
    context "when notice event employer_initial_eligibility_denial_notice is received" do
      ...
      it "should return renewal plan year warnings" do
        expect(merge_model.plan_year.warnings).to eq model_instance.application_eligibility_warnings[:fte_count]
      end
    end
  end
end
