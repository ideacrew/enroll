...
describe EmployerProfile, dbclean: :after_each do
  ...
  context "trigger renewal_employer_invoice_available" do
    let(:params)  { {} }
    let(:employer_profile) {EmployerProfile.new(**params)}
    it "should trigger renewal_notice job in queue" do
      ActiveJob::Base.queue_adapter = :test
      ActiveJob::Base.queue_adapter.enqueued_jobs = []
      employer_profile.trigger_notices("renewal_employer_invoice_available")
      queued_job = ActiveJob::Base.queue_adapter.enqueued_jobs.find do |job_info|
        job_info[:job] == ShopNoticesNotifierJob
      end
      expect(queued_job[:args]).to eq [employer_profile.id.to_s, 'renewal_employer_invoice_available']
    end
  end
