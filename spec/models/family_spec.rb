...
describe Family do
  ...
  context "earliest_effective_shop_sep" do
    ...
    let(:effective_date)         { Date.new(calender_year-1, 12, 1)  } # no valid sep case - when effective date of sep falls outside employer's coverage period.
    ...
    let(:benefit_group_assignment) { BenefitGroupAssignment.create({ census_employee: census_employee, benefit_group: plan_year.benefit_groups.first, start_on: plan_year_start_on})}
    ...
    let(:published_plan_year)  { FactoryGirl.build(:plan_year, aasm_state: :published, start_on: plan_year_start_on)}
    ...
    let(:special_enrollment_period) { FactoryGirl.create(:special_enrollment_period, effective_on: plan_year_start_on,family: shop_family)}
    
    before do
      date1 = TimeKeeper.date_of_record - 30.days
      @current_sep = FactoryGirl.build(:special_enrollment_period, qle_on: date1, effective_on: effective_date)
      shop_family.special_enrollment_periods.shop_market << @current_sep
      employer_profile.plan_years << published_plan_year
      employer_profile.save

      family.special_enrollment_periods << @current_sep
      @another_current_sep = FactoryGirl.build(:special_enrollment_period, qle_on: 4.days.ago.to_date, effective_on: effective_date)
      shop_family.special_enrollment_periods << @another_current_sep
      shop_family.save 
    end

    it "should return a sep with an effective date that equals to employers plan year start-date when there is no valid sep" do
      expect(@current_sep.is_active?).to eq true
      allow(shop_enrollment).to receive(:employee_role).and_return(employee_role)
      allow(person).to receive(:primary_family).and_return(shop_family)
      expect(shop_family.earliest_effective_shop_sep.effective_on).to eq published_plan_year.start_on
    end
