    ...
    context "#difference" do
      context "and identical records are compared" do
        it "should return empty hash" do
          expect(family_transcript.difference(base_hash, base_hash)).to eq Hash.new
        end

        context "and the identical records contain an array" do
          it "should return empty hash" do
            # expect(family_transcript.difference(base_array, base_array)).to eq Hash.new
          end
        end
      end
      ...
      context "and the base record has a value missing from the compare record" do
        it "should return a remove for that key" do
          expect(family_transcript.difference(base_hash, base_hash.except(:occupation))).to eq occupation_dropped
        end
      end

      context "and the compare record has a value missing from the base record" do
        it "should return an add for that key" do
          expect(family_transcript.difference(base_hash.except(:last_name), base_hash)).to eq last_name_added
        end
      end

      context "and the compare record updates a base record value" do
        it "should return an update for that key" do
          expect(family_transcript.difference(base_hash, occupation_update)).to eq occupation_updated
        end
      end
      ...
      context "and multiple values are updated between the base and compare records" do
        it "should return an update for multiple keys" do
          expect(family_transcript.difference(base_hash, multi_update)).to eq multi_updated
        end

        context "and the records contain an array with changes" do
          it "should return an update for multiple keys" do
            expect(family_transcript.difference(base_array, compare_array)).to eq array_changes
          end
        end
      end
