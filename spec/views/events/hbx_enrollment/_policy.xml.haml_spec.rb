...
RSpec.describe 'events/hbx_enrollment/_policy.xml.haml', dbclean: :after_each do
  ...
  context 'for dependent enrollment only' do
    ...
    before do
      EnrollRegistry[:enroll_app].setting(:send_rating_area_to_edi).stub(:item).and_return(false)
    end
  ...
  context 'enrollment with rating area' do
    ...
    before do
      EnrollRegistry[:enroll_app].setting(:send_rating_area_to_edi).stub(:item).and_return(true)
