...
#   let(:hbx_enrollment) { HbxEnrollment.new(created_at: TimeKeeper.date_of_record, effective_on: TimeKeeper.date_of_record) }
  let(:hbx_enrollment) { FactoryBot.create(:hbx_enrollment, family: family) }
  let(:term_hbx_enrollment) { HbxEnrollment.new(created_at: TimeKeeper.date_of_record, effective_on: TimeKeeper.date_of_record, aasm_state: "coverage_terminated") }
...
#   end
  ...
  context 'user is HBX admin' do
    before do
      ...
      assign(:family, family)
      assign(:all_hbx_enrollments_for_admin, [hbx_enrollment, term_hbx_enrollment])
    end
    ...
    it "should display select box to show all enrollments" do
      ...
      render file: "insured/families/enrollment_history.html.erb"
      binding.irb
