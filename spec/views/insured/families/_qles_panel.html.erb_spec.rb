...
RSpec.describe "insured/families/_qles_panel.html.erb" do
  ...
  context 'update not authorized' do
    ...
    it "should display the title" do
      expect(rendered).not_to have_selector('h4', text: 'Have life changes?')
      expect(rendered).not_to have_selector('small', text: 'See how it may affect your health insurance.')
      expect(rendered).not_to have_selector('h5', text: 'TOP LIFE CHANGES')
    end

    it "should have carousel" do
      expect(rendered).not_to have_selector('div#carousel-qles')
      expect(rendered).not_to have_selector('div.carousel-inner')
      expect(rendered).not_to have_selector('ol.carousel-indicators')
      expect(rendered).not_to have_selector('a.carousel-control', count: 2)
    end
