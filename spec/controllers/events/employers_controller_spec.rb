...
describe Events::EmployersController do
  describe "#resource with an employer_id" do
    ...
    describe "#resource with an employer_id and benefit_application_id" do
      let(:props_with_plan_year_id) { double(:headers => {:employer_id => employer_hbx_id, :benefit_application_id => "benefit_application_id"}, :reply_to => reply_to_key) }
      let(:found_orgs) { [employer_org] }
      before :each do
        allow(BenefitSponsors::Organizations::Organization).to receive(:employer_by_hbx_id).with(employer_hbx_id).and_return(found_orgs)
        allow(controller).to receive(:render_to_string).with(
