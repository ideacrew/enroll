...
describe FakesController do
  ...
  describe 'clear_history_noise_if_present_from' do
    let(:person) { FactoryBot.create(:person, consumer_role: consumer_role) }

    context 'with citizenship type' do
      let(:consumer_role) { FactoryBot.build(:consumer_role) }
      let(:citizenship_verification) { person.verification_types.where(type_name: 'Citizenship').first }

      context "with 'NO VLP Documents' type history element as latest" do
        let(:add_to_citizenship_verification) do
          citizenship_verification.type_history_elements.create!(
            action: "Hub Request Failed",
            modifier: "System",
            update_reason: "Citizenship Request Failed due to ['No VLP Documents']"
          )
        end

        before do
          add_to_citizenship_verification
          subject.clear_history_noise_if_present_from(consumer_role)
        end

        it "deletes type history element with update_reason 'NO VLP Documents'" do
          expect(citizenship_verification.reload.type_history_elements).to be_empty
        end
      end

      context "with 'NO VLP Documents' type history element as not the latest" do
        let(:add_to_citizenship_verification) do
          citizenship_verification.type_history_elements.create!(
            action: "Hub Request Failed",
            modifier: "System",
          ...
          citizenship_verification.type_history_elements.create!(
            action: "Hub Request Failed",
            modifier: "System",
            update_reason: "Citizenship Request Failed due to ['Invalid VLP Documents']"
          )
        end
        ...
        it "does not delete type history element with update_reason 'NO VLP Documents' as it is not the latest" do
          expect(citizenship_verification.reload.type_history_elements.count).to eq(2)
        end
      end

      context "without 'NO VLP Documents' type history element" do
        let(:add_to_citizenship_verification) do
          citizenship_verification.type_history_elements.create!(
            action: "Hub Request Failed",
            modifier: "System",
        ...
        it "does not delete any type history elements" do
          expect(citizenship_verification.reload.type_history_elements.count).to eq(1)
        end
      end

      context "without any type history elements" do
        before do
          subject.clear_history_noise_if_present_from(consumer_role)
        end

        it "does not change any type history elements" do
          expect(citizenship_verification.reload.type_history_elements).to be_empty
        end
      end
    end

    context 'with Immigration status type' do
      let(:consumer_role) { FactoryBot.build(:consumer_role, citizen_status: 'alien_lawfully_present') }
      let(:immigration_verification) { person.verification_types.where(type_name: 'Immigration status').first }

      context "with 'NO VLP Documents' type history element as latest" do
        let(:add_to_immigration_verification) do
          immigration_verification.type_history_elements.create!(
            action: "Hub Request Failed",
            modifier: "System",
            update_reason: "Immigration status Request Failed due to ['No VLP Documents']"
          )
        end

        before do
          add_to_immigration_verification
          subject.clear_history_noise_if_present_from(consumer_role)
        end

        it "deletes type history element with update_reason 'NO VLP Documents'" do
          expect(immigration_verification.reload.type_history_elements).to be_empty
        end
      end
      ...
      context "with 'NO VLP Documents' type history element as not the latest" do
        let(:add_to_immigration_verification) do
          ...
          immigration_verification.type_history_elements.create!(
            action: "Hub Request Failed",
            modifier: "System",
            update_reason: "Immigration status Request Failed due to ['Invalid VLP Documents']"
          )
        end
        ...
        it "does not delete type history element with update_reason 'NO VLP Documents' as it is not the latest" do
          expect(immigration_verification.reload.type_history_elements.count).to eq(2)
        end
      end
      ...
      context "without 'NO VLP Documents' type history element" do
        ...
        it "does not delete any type history elements" do
          expect(immigration_verification.reload.type_history_elements.count).to eq(1)
        end
      end
    ...
    context 'without Citizenship, Immigration status verification types' do
      let(:consumer_role) { FactoryBot.build(:consumer_role, is_applying_coverage: false, citizen_status: nil) }

      it 'does not raise when processing without the verification types' do
        expect { subject.clear_history_noise_if_present_from(consumer_role) }.not_to raise_error
      end
    end
  end
end
