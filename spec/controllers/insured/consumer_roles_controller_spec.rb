...
RSpec.describe Insured::ConsumerRolesController, dbclean: :after_each, :type => :controller do
  ...
  context "POST create with failed construct_employee_role", dbclean: :after_each do
    let(:person_params){{"dob"=>SymmetricEncryption.encrypt("1985-10-01"), "first_name"=>"martin","gender"=>"male","last_name"=>"york","middle_name"=>"","name_sfx"=>"","ssn"=>SymmetricEncryption.encrypt("000000111"),"user_id"=>"xyz"}}
    let(:person_user){ double("User") }
    before(:each) do
