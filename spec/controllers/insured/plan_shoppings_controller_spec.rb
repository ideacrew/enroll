...
RSpec.describe Insured::PlanShoppingsController, :type => :controller, dbclean: :after_each do
  ...
  if ExchangeTestingConfigurationHelper.individual_market_is_enabled?
    describe "plan_selection_callback" do
      ...
      let(:hios_id_with_year) { "#{year}-#{product.hios_id}" }
      ...
      context "When a callback is received" do
        before do
          sign_in user
          get :plan_selection_callback, params: { id: hbx_enrollment.id, hios_id: hios_id_with_year, market_kind: market_kind, coverage_kind: coverage_kind }
        end
