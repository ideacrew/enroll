...
RSpec.shared_context "setup family initial and renewal enrollments data", :shared_context => :metadata do
  ...
  let!(:spouse_rec) do
    FactoryBot.create(:person, :with_consumer_role, dob: spouse_dob, is_tobacco_user: 'y')
  end
  ...
  let!(:child1) do
    child = FactoryBot.create(:person, :with_consumer_role, dob: child1_dob)
    FactoryBot.create(:family_member, person: child, family: family)
  end

  let!(:child2) do
    child = FactoryBot.create(:person, :with_consumer_role, dob: child2_dob)
    FactoryBot.create(:family_member, person: child, family: family)
  end

  let!(:child3) do
    child = FactoryBot.create(:person, :with_consumer_role, dob: child3_dob)
    FactoryBot.create(:family_member, person: child, family: family)
  end
