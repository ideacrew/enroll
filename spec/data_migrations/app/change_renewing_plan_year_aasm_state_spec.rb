...
describe ChangeRenewingPlanYearAasmState, dbclean: :after_each do
  ...
  let!(:rate_reference) { FactoryGirl.create(:rating_area)  }
  ...
  describe "updating aasm_state of the renewing plan year", dbclean: :after_each do
    ...
    let!(:rate_reference) { create(:rating_area, county_name: organization.primary_office_location.address.county, zip_code: organization.primary_office_location.address.zip)}
