...
describe UnsetBenefitGroupAssignment, dbclean: :after_each do
  ...
  pending("FIXME: update examples") do 
    ...
    describe "creates an inactive benefit group assignment" do
      ...
      before(:each) do
        ...
        census_employee.benefit_group_assignments.last.update(is_active:true)
      end
      ...
      context "updating benefit group assignment to active", dbclean: :after_each do
        ...
        it "should make the benefit group assignment active" do
          expect(census_employee.benefit_group_assignments.last.is_active).to eq(true)
          subject.migrate
          census_employee.reload
          expect(CensusEmployee.find(census_employee.id).benefit_group_assignments.last.is_active).to eq(false)
        end
      end
    end
  end
