...
describe EnrollmentDataUpdate do
  ...
  describe "shop enrollment case " do
    ...
    let!(:hbx_enrollment1) { FactoryGirl.create(:hbx_enrollment, :shop, benefit_group_id: benefit_group.id,household: family.active_household,effective_on: 1.month.ago.to_date, terminated_on: TimeKeeper.datetime_of_record-1, aasm_state: "coverage_canceled")}
    let!(:hbx_enrollment2) { FactoryGirl.create(:hbx_enrollment,  :shop,benefit_group_id: benefit_group.id,household: family.active_household,effective_on: 1.month.ago.to_date+1.days,submitted_at:1.month.ago.to_date-1.days)}
    let!(:hbx_enrollment_member1) {FactoryGirl.create(:hbx_enrollment_member,hbx_enrollment: hbx_enrollment1,applicant_id: family.primary_family_member.id,eligibility_date: Date.new(2016,1,1))}
    let!(:hbx_enrollment_member2) {FactoryGirl.create(:hbx_enrollment_member,hbx_enrollment: hbx_enrollment2,applicant_id: family.primary_family_member.id,eligibility_date: Date.new(2016,1,1))}
    before(:each) do
