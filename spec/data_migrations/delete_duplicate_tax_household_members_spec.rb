...
describe DeleteDuplicateTaxHouseholdMembers, dbclean: :after_each do
  let(:given_task_name) { 'delete_duplicate_tax_household_members' }

  subject { DeleteDuplicateTaxHouseholdMembers.new(given_task_name, double(:current_scope => nil)) }
  ...
  describe 'for a given hxb_id' do
    ...
    context 'for a valid person hbx_id' do
      let(:hbx_id) { 'hbx_id' }
    ...
    context 'has duplicate tax household members' do
      ...
      let(:tax_household_member2) { FactoryBot.create(:tax_household_member, tax_household: tax_household, family_member: person.primary_family.family_members.first) }

      before do
        tax_household.tax_household_members << tax_household_member
        tax_household.tax_household_members << tax_household_member2
      ...
      it 'should delete duplicate tax households' do
        expect { person.primary_family.active_household.tax_households.first.tax_household_members.to eq(1) }
      end
    end
  end
end
