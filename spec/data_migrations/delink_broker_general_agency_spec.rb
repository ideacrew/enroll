...
describe DelinkBrokerGeneralAgency do
  ...
  describe "delink general and broker agency" do
    let(:address_01) { Address.new(kind: 'primary', address_1: '106 Autumn Hill Way', address_2: '', address_3: '', city: 'Gaithersburg', county: nil, state: 'MD', location_state_code: nil, zip: '20877', country_name: '', full_text: nil) }
    let(:phone_01) { Phone.new(kind: 'phone main', country_code: '', area_code: '301', number: '9227984', extension: '', primary: nil, full_phone_number: '3019227984') }
    let(:office_location_01) { OfficeLocation.new(is_primary: true, address: address_01, phone: phone_01) }
    let!(:organization){  Organization.new(:legal_name => "Alvin Frager", :fein => "521698168", :dba => "Alvin Frager Insurance", office_locations: [office_location_01]) }
