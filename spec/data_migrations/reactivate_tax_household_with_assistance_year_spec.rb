...
describe ReactivateTaxHouseholdWithAssistanceYear, dbclean: :after_each do
  ...
  describe "Set the effective end on to nil" do
    ...
    let(:eligibility_determination) {EligibilityDetermination.new(source: 'Admin', csr_eligibility_kind: 'csr_73', max_aptc: 1017, csr_percent: 0.73, determined_on: TimeKeeper.date_of_record)}
    ...
    around do |example|
      ClimateControl.modify primary_person_hbx_id: person.hbx_id,
                            applicable_year: TimeKeeper.date_of_record.year.to_s,
                            max_aptc: '1017',
                            csr_percent: '73' do
        example.run
      end
  ...
  describe "Should not set the effective end on to nil" do
    ...
    let(:household) {Household.new}
    let(:family) { FactoryBot.create(:family, :with_primary_family_member, person: person, households: [household])}
    let(:eligibility_determination) {EligibilityDetermination.new(source: 'Admin', csr_eligibility_kind: 'csr_73', max_aptc: 100, csr_percent: 0.73, determined_on: TimeKeeper.date_of_record)}
    let(:tax_household) {TaxHousehold.new(effective_starting_on: TimeKeeper.date_of_record, effective_ending_on: TimeKeeper.date_of_record+1.year, eligibility_determinations: [eligibility_determination])}
  ...
  describe "Should not set the effective end on to nil" do
    ...
    let(:household) {Household.new}
    let(:family) { FactoryBot.create(:family, :with_primary_family_member, person: person, households: [household])}
    let(:eligibility_determination) {EligibilityDetermination.new(source: 'Admin', csr_eligibility_kind: 'csr_87', max_aptc: 100, csr_percent: 0.87, determined_on: TimeKeeper.date_of_record)}
    let(:tax_household) {TaxHousehold.new(effective_starting_on: TimeKeeper.date_of_record, effective_ending_on: TimeKeeper.date_of_record+1.year, eligibility_determinations: [eligibility_determination])}
