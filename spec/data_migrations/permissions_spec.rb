...
describe DefinePermissions do
  describe 'create permissions' do
    ...
    subject { DefinePermissions.new(given_task_name, double(:current_scope => nil)).initial_hbx}	
    before do
      Person.all.delete
  ...
  describe 'build test roles' do
    ...
    let(:roles) {%w{hbx_staff hbx_read_only hbx_csr_supervisor hbx_csr_tier2 hbx_csr_tier1} }
    subject { DefinePermissions.new(given_task_name, double(:current_scope => nil)).build_test_roles}  
    before do
      User.all.delete
      Person.all.delete
