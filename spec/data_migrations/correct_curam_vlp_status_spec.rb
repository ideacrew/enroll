...
describe CorrectCuramVlpStatus do
  ...
  describe "given a person who has a vlp authority of 'curam', in the 'pending' status" do
    ...
    context "people with NO SSN" do
      let(:curam_user1) {
        person = FactoryGirl.build(:person, :with_consumer_role)
        person.ssn = nil
      ...
      before :each do
        curam_user1.consumer_role.lawful_presence_determination.vlp_authority = "curam"
        curam_user1.consumer_role.lawful_presence_determination.aasm_state = "verification_pending"
        curam_user1.consumer_role.aasm_state = "verification_outstanding"
        curam_user1.save!
        subject.migrate
        curam_user1.reload
      end
      ...
      it "moves SSN to 'valid' state" do
        expect(curam_user1.consumer_role.ssn_validation).to eq 'na'
      end

      it "does not save ssn_update_reason" do
        expect(curam_user1.consumer_role.ssn_update_reason).to eq nil
      end

      it "keeps vlp authority as curam" do
        expect(curam_user1.consumer_role.lawful_presence_determination.vlp_authority).to eq "curam"
      end

      it "moves lpd state to 'verification_successful'" do
        expect(curam_user1.consumer_role.lawful_presence_determination.aasm_state).to eq "verification_successful"
      end

      it "saves 'lawful_presence_update_reason' as Hash with update_reason and update_comment" do
        expect(curam_user1.consumer_role.lawful_presence_update_reason[:update_reason]).to eq "user in curam"
        expect(curam_user1.consumer_role.lawful_presence_update_reason[:update_comment]).to eq "fix data migration"
      end
    end
  end
