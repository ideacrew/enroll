...
describe ForcePublishPlanYears, dbclean: :after_each do
    ...
    before :each do
      ...
      allow(ENV).to receive(:[]).with("start_on_date").and_return start_on_date
      allow(ENV).to receive(:[]).with("current_date").and_return current_date
    end
    ...
    let!(:start_on_date)      { Date.new(2018,12,1) }
    ...
    let(:given_task_name) { "force_publish_plan_years" }

    subject {ForcePublishPlanYears.new(given_task_name, double(:current_scope => nil)) }
    ...
    describe "force publish process" do
      ...
      it 'reverts plan years from renewing published to renewing draft' do 
        ...
        subject.revert_plan_years( start_on_date, current_date)
      ...
      it 'assigns benefit packages to CEs that are not assigned' do 
        ...
        subject.assign_packages(start_on_date, current_date)
      ...
      it 'sets back the OE date for renewing draft employers with a OE date greater than current date' do
        ...
        subject.set_back_oe_date(start_on_date, current_date)
      ...
      it 'force publishes plan years from renewing draft to renwing enrolling' do 
        ...
        subject.force_publish(start_on_date, current_date)
      ...
      it 'logs plan years not in renewing enrolling to a csv not_enrolling_file' do
        subject.clean_up( start_on_date, current_date)
