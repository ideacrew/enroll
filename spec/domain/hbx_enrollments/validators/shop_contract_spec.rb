...
RSpec.describe HbxEnrollments::Validators::ShopContract, dbclean: :after_each do
  let(:params) do
  ...
  context 'for failure case' do
    before do
      @result = subject.call(params.except(:sponsored_benefit_package_id))
    end
    ...
    it 'should return any errors' do
      expect(@result.errors.to_h).to eq({:sponsored_benefit_package_id => ["is missing"]})
    end
  end
end