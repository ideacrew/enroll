...
RSpec.describe ::Operations::Products::FetchSlcspPremiumForTaxHouseholdMember, dbclean: :after_each do
  ...
  describe 'clients with county based rating area' do
    ...
    it 'should return premium amount & product for the member' do
      result = subject.call(params)
      expect(result.success).to eq({ :cost => 200.0, :product_id => product.id })
    end
  end
end
