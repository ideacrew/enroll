...
RSpec.describe ::Operations::Products::FetchLcsp, dbclean: :after_each do
  ...
  describe 'valid params' do
    ...
    it 'should return an array of health_only_lcsp_premiums for the given family' do
      value = subject.call(params).value!
      expect(value.is_a?(Hash)).to eq true
      expect(value[person.hbx_id].keys.include?(:health_only_lcsp_premiums)).to eq true
    end

    it 'should return details of lowest plan' do
      values = subject.call(params).value![person.hbx_id][:health_only_lcsp_premiums]
      expect(values[:monthly_premium]).to eq 200.0
