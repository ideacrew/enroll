...
RSpec.describe ::Operations::HbxEnrollments::BeginCoverageHandler, dbclean: :after_each do
  ...
  let(:benefit_coverage_period) do
    FactoryBot.create(:hbx_profile, :open_enrollment_coverage_period).benefit_sponsorship.current_benefit_coverage_period
  end
  let(:start_on) { benefit_coverage_period.start_on }
  let(:end_on) { benefit_coverage_period.end_on }
  ...
  let(:logger_content) do
    File.read("#{Rails.root}/log/hbx_enrollments_begin_coverages_handler_#{TimeKeeper.date_of_record.strftime('%Y_%m_%d')}.log")
  end
  ...
  describe 'with valid params' do
    let(:success_msg) do
      'Done publishing enrollment begin coverage events. See hbx_enrollments_begin_coverages_handler log for results.'
    end
  ...
  after :all do
    file_path = "#{Rails.root}/log/hbx_enrollments_begin_coverages_handler_#{TimeKeeper.date_of_record.strftime('%Y_%m_%d')}.log"
