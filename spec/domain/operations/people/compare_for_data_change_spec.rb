    ...
    context 'for incoming payload and existing attributes are same but attributes order changed' do
      let(:person_params) do
        {:hbx_id=>'13cce9fe14b04209b2443330900108d8',
         ...
         last_name: 'childlast',
         gender: 'male',
         :addresses=>[person.mailing_address.serializable_hash.symbolize_keys.except(:_id, :created_at, :updated_at, :tracking_version, :full_text, :location_state_code, :modifier_id),
                      person.home_address.serializable_hash.symbolize_keys.except(:_id, :created_at, :updated_at, :tracking_version, :full_text, :location_state_code, :modifier_id)]}
      end
...
def fetch_array_of_attrs_for_embeded_objects(data)
  ...
  data.each do |special_hash|
    new_arr << special_hash.symbolize_keys.except(:_id, :created_at, :updated_at, :tracking_version, :full_text, :location_state_code, :modifier_id)
  end
  new_arr
end
