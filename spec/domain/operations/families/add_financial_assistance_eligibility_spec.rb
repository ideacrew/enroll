...
RSpec.describe Operations::Families::AddFinancialAssistanceEligibilityDetermination, type: :model, dbclean: :after_each do
  ...
  # let!(:applicant1) {FactoryBot.create(:financial_assistance_applicant, eligibility_determination_id: eligibility_determination1.id, application: application, family_member_id: family.primary_applicant.id)}
  let!(:params){{:family_id=>BSON::ObjectId(family.id),
