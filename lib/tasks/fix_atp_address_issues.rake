...
namespace :migrations do
  ...
  task :fix_atp_address_issues, [:file] => :environment do |_task, _args|
      ...
      CSV.open(file_name, 'w+', headers: true) do |csv|
        ...
        while counter < number_of_iterations
          ...
          output = @people.no_timeout.offset(offset_count).each do |person|
            ...
            mailing = person.addresses.detect {|a| a.kind == "mailing" }
            updated_address_hash = person.addresses.reject { |a| a[:kind] == "mailing" } if !mailing.nil? && matching_address(person)
          ...
          output = @people.no_timeout.offset(offset_count).each do |person|
            ...
            @target_app.no_timeout.select{|application| application.no_timeout.applicants.no_timeout.detect{|a| a.person_hbx_id == person.hbx_id}}&.each do |application|
              ...
              application.no_timeout.applicants.no_timeout.select{|a| a.person_hbx_id == person.hbx_id}.each do |applicant|
                ...
                a_updated_address_hash = applicant.addresses.reject { |a| a[:kind] == "mailing" } if !a_mailing.nil? && matching_address(applicant)
