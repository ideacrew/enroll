namespace :import do
  desc "import ivl benefit_package for 2018"
  task :ivl_benefit_package_for_2018 => :environment do
    puts "::: Creating IVL 2018 benefit packages :::"
    # BenefitPackages - HBX 2017
    hbx = HbxProfile.current_hbx

    # Second lowest cost silver plan
    # slcsp_2017 = Plan.where(active_year: 2018).and(hios_id: "86052DC0400001-01").first
    ...
    unless bc_period = hbx.benefit_sponsorship.benefit_coverage_periods.detect { |bcp| bcp.start_on.year == 2018 }
      bc_period = bc_period_2017.dup
      bc_period.title = "Individual Market Benefits 2018"
      bc_period.start_on = bc_period_2017.start_on + 1.year
      bc_period.end_on = bc_period_2017.end_on + 1.year
      bc_period.open_enrollment_start_on = bc_period_2017.open_enrollment_start_on + 1.year
      bc_period.open_enrollment_end_on = Date.new(2018,1,31)
      # bc_period.slcsp = slcsp_2017.try(:id)
      # bc_period.slcsp_id = slcsp_2017.try(:id)
    ...
    # bc_period.save!
    puts "import benefit_package for 2018 successful."
  end
end
