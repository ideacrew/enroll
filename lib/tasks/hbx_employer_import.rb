...
def Employer.parse_date_safe(field)
  date = nil
  if field.present?
    begin
...
def Employer.from_row(row)
  ...
  employer.complete_date               = parse_date_safe(row[2])
  employer.user_name                   = row[3]
  employer.company_external_id         = row[4].to_i
  employer.fein                        = row[5].to_s.gsub(/\D/, '')
  employer.dba                         = row[6]
  employer.legal_name                  = row[7]

  employer.previous_effective_date     = row[8]
  employer.effective_date              = parse_date_safe(row[9])
  employer.open_enrollment_start       = parse_date_safe(row[10])
  employer.open_enrollment_end         = parse_date_safe(row[11])

  employer.poc_first_name              = row[12]
  employer.poc_last_name               = row[13]
  employer.poc_email                   = row[14]
  employer.poc_phone                   = row[15].to_s.gsub(/\D/, '')
  ...
  employer.assign_enter_date           = parse_date_safe(row[101])
  employer.assign_end_date             = parse_date_safe(row[102])
  employer
end

Employer.send(:define_method, :create_or_update_organization) do
