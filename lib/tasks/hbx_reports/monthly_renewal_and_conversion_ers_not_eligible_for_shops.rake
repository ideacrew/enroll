# Monthly implementation list of renewal and conversion employers that do not meet eligibility to participate in SHOP
...
#e.g.,given that the report in generated on 12/14 only 1/1 ERs will populate on the report
#call this rake task as:rake reports:shop:initial_ERs_not_qualified_to_shop["01/01/2016"]

require 'csv'
namespace :reports do
  namespace :shop do
    desc "List of renewal_conversion_ERs not_meet eligibility to participate in shop"
    task :renewal_and_conversion_ERs_not_qualified_to_shop,[:effective_date] => :environment do |task, args|
      count = 0
      effective_date = Date.strptime(args[:effective_date], "%m/%d/%Y")
      initial_employers=Organization.where(:"employer_profile.plan_years" => { :$elemMatch => { :"start_on" => effective_date, :"aasm_state".in => PlanYear::PUBLISHED}})
      field_names  = %w(
                        Employer_Name
                        Employer_Fein
                        Employee_HBX_ID
      ...
      CSV.open(file_name, "w") do |csv|
        ...
        puts "Waiver Report for Employees Generated on #{Date.today}, Total renewal and conversion ERs Employers count #{count} and Employer information output file: #{file_name}"
      end # CSV close
    end
  end
end