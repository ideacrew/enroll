# Monthly implementation list of renewal/conversion ERs that do not meet eligibility to participate in SHOP
# Business Rules
...
namespace :reports do
  namespace :shop do
    ...
    task :initial_ERs_not_qualified_to_shop => :environment do
      ...
      initial_employers = Organization.where(:"start_on" => Date.new(2016,2,1),:"employer_profile.plan_years" => {:$elemMatch => { :aasm_state.in => PlanYear::RENEWING_PUBLISHED_STATE}})
      field_names  = %w(
                        Employer_Name
                        Employer_Fein
                        Employee_HBX_ID
      ...
      CSV.open(file_name, "w") do |csv|
        ...
        puts "Waiver Report for Employees Generated on #{Date.today}, Total renew/conversion Employers count #{count} and Employer information output file: #{file_name}"
      end # CSV close
    end
  end
end