  ...
  namespace :shop do
    ...
    task :brokers => :environment do
      ...
      file_name = if individual_market_is_enabled?
                    File.expand_path("#{Rails.root}/brokers_list_#{time_stamp}.csv")
                  else
                    ...
                    File.expand_path("#{Rails.root}/CCA_#{ENV["RAILS_ENV"]}_Broker_#{date_extract[0]}_#{date_extract[1]}_#{date_extract[2]}_#{fetch_day}_#{time_extract[0]}_#{time_extract[1]}_#{time_extract[2]}.csv")
                  end
