  ...
  namespace :shop do
    ...
    task :employee_terminations => :environment do
      ...
      file_name = if individual_market_is_enabled?
                    ...
                    File.expand_path("#{Rails.root}/CCA_#{ENV["RAILS_ENV"]}_employee_terminations_#{date_extract[0]}_#{date_extract[1]}_#{date_extract[2]}_#{fetch_day}_#{time_extract[0]}_#{time_extract[1]}_#{time_extract[2]}.csv")
                  end
