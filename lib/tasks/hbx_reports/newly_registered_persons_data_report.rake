...
namespace :reports do
  namespace :new_people do
    ...
    task :total_new_people_list => :environment do
      ...
      start_date = ENV["start_date"] ? Date.strptime(ENV["start_date"], "%m/%d/%Y") : TimeKeeper.date_of_record - 7.days
      end_date = ENV["end_date"] ? Date.strptime(ENV["end_date"], "%m/%d/%Y") : TimeKeeper.date_of_record + 1.day
