     ...
     task :total_new_people_list => :environment do
      ...
      end_date = TimeKeeper.date_of_record + 1.day
       ...
       file_name = if individual_market_is_enabled?
                     File.expand_path("#{Rails.root}/public/new_registered_persons_data_report_#{time_stamp}.csv")
                   else
                     ...
                     File.expand_path("#{Rails.root}/CCA_#{ENV["RAILS_ENV"]}_new_registered_persons_data_report_#{date_extract[0]}_#{date_extract[1]}_#{date_extract[2]}_#{fetch_day}_#{time_extract[0]}_#{time_extract[1]}_#{time_extract[2]}.csv")
                   end
