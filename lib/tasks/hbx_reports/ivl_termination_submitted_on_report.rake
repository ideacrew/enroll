...
namespace :reports do
  namespace :ivl do
    ...
    task :terminations_submitted_on => :environment do
      ...
      field_names  = %w(
               HBX_ID,Enrolled_Member_First_Name,Enrolled_Member_Last_Name
               Primary_Member_HBX_ID,Primary_Member_First_Name,Primary_Member_Last_Name
               Market_Kind
               Plan_Name
               HIOS_ID
               Policy_ID
      ...
      CSV.open(file_name, "w", force_quotes: true) do |csv|
        ...
        families.each do |family|
          ...
          hbx_enrollments = [family].flat_map(&:households).flat_map(&:hbx_enrollments).select{|hbx| hbx.aasm_state == "coverage_terminated" && hbx.termination_submitted_on.strftime('%Y-%m-%d') == Date.yesterday.strftime('%Y-%m-%d')}
          ...
          hbx_enrollment_members.each do |hbx_enrollment_member|
            if hbx_enrollment_member
              csv << [
                  hbx_enrollment_member.person.hbx_id + "," + hbx_enrollment_member.person.first_name + "," + hbx_enrollment_member.person.last_name,
                  family.primary_family_member.person.hbx_id + "," + family.primary_family_member.person.first_name + "," + family.primary_family_member.person.last_name,
