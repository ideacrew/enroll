namespace :import do
  ...
  task :create_2023_ivl_packages_ME  => :environment do
    ...
    ivl_health_and_non_cat_plans_2023 = ivl_products.where( kind: "health", hios_id: /-01$/ ).not_in(metal_level_kind: "catastrophic").select{|a| a.active_year == 2023}.entries.collect(&:_id)
    ivl_dental_plans_2023             = ivl_products.where( kind: "dental").select{|a| a.active_year == 2023}.entries.collect(&:_id)
    ivl_health_plans_2023             = ivl_products.where( kind: "health", hios_id: /-01$/ ).select{|a| a.active_year == 2023}.entries.collect(&:_id)
    ivl_and_cat_health_plans_2023     = ivl_products.where( kind: "health", hios_id: /-01$/ ).where(metal_level_kind: "catastrophic").select{|a| a.active_year == 2023}.entries.collect(&:_id)

    individual_health_and_non_cat_benefit_package = BenefitPackage.new(
        title: "individual_health_and_non_catastrophic_benefits_2023",
        elected_premium_credit_strategy: "unassisted",
        benefit_ids:          ivl_health_and_non_cat_plans_2023,
