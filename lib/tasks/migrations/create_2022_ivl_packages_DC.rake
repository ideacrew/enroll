namespace :import do
  ...
  task :create_2022_ivl_packages_DC  => :environment do
    ...
    if bc_period_2022.present?
      ...
      bc_period_2022.start_on = bc_period_2021.start_on + 1.year
      bc_period_2022.end_on = bc_period_2021.end_on + 1.year
    ...
    ivl_products = BenefitMarkets::Products::Product.by_year(2022).aca_individual_market.with_premium_tables
    puts 'no ivl product present' if ivl_products.blank?
    abort if ivl_products.blank?

    ivl_health_plans_2022         = ivl_products.where( kind: "health", hios_id: /-01$/ ).not_in(metal_level_kind: "catastrophic").pluck(:_id)
    ivl_dental_plans_2022         = ivl_products.where( kind: "dental").pluck(:_id)
    ivl_and_cat_health_plans_2022 = ivl_products.where( kind: "health", hios_id: /-01$/ ).pluck(:_id)
