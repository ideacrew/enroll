...
csv << %w(family.id policy.id policy.subscriber.coverage_start_on policy.aasm_state policy.plan.coverage_kind policy.plan.metal_level policy.plan.plan_name policy.total_premium deductible family_deductible  person.hbx_id person.first_name person.last_name
...
def add_to_csv(csv, policy, person, is_dependent, outstanding_verification_types, document_due_date)
  csv << [policy.family.id, policy.hbx_id, policy.effective_on, policy.aasm_state, policy.plan.coverage_kind, policy.plan.metal_level, policy.plan.name, policy.total_premium, policy.plan.deductible, policy.plan.family_deductible.split("|").last.squish, person.hbx_id, person.first_name, person.last_name,
...
while offset < family_count
  Family.offset(offset).limit(batch_size).flat_map(&:households).flat_map(&:hbx_enrollments).each do |policy|
    begin
      ...
      add_to_csv(csv, policy, person, false, check_for_outstanding_verification_types(person), document_due_date(policy.family))

      policy.hbx_enrollment_members.each do |hbx_enrollment_member|
        add_to_csv(csv, policy, hbx_enrollment_member.person, true, check_for_outstanding_verification_types(hbx_enrollment_member.person), document_due_date(policy.family)) if hbx_enrollment_member.person != person
      end
    rescue => e
