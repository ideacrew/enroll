...
def document_due_date(family)
  enrolled_contingent_enrollment = family.enrollments.where(is_any_enrollment_member_outstanding: true, :kind => 'individual').first
...
def is_family_renewing(family)
  family.active_household.hbx_enrollments.where(:aasm_state.in => ["coverage_selected"], kind: "individual", effective_on: Date.new(2017,1,1)).present?
end
