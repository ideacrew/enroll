...
Organization.where(:fein.in => feins).each do |organization|
  ...
  Family.where(:"households.hbx_enrollments" => {:$elemMatch => {:benefit_group_id.in => bg_ids, :aasm_state.in => (HbxEnrollment::ENROLLED_STATUSES + HbxEnrollment::WAIVED_STATUSES)}}).each do |family|
