...
def check_and_run
  ...
  CSV.foreach("pids/#{start_date_of_next_year.year}_THHEligibility.csv") do |row_with_ssn|
    ...
    if person_ssn && person_ssn =~ /^\d+$/ && person_ssn.to_s != '0'
      ssn = '0'*(9-person_ssn.length) + person_ssn if person_ssn.length < 9
    ...
    if person_ssn && person && person_by_hbx && person.id != person_by_hbx.id
      not_run << {person_ssn: person_ssn, hbx_id: primary_hbx_id, error: "Bad CSV data(person_ssn/hbx_id)"}
      puts "Person with ssn: #{person_ssn} and person with hbx_id: #{primary_hbx_id} are not same"
      next
    end
    ...
    if primary_person
      ...
      unless primary_family
        not_run << {person_hbx_id: person.hbx_id, primary_hbx_id: primary_person_hbx_id}
        puts "Family not found for primary person with hbx_id: #{primary_hbx_id} when processing for person hbx_id: #{person_hbx_id}"
        next
      end
    else
      not_run << {error: 'did not find the primary family for the primary person with hbx_id #{primary_hbx_id}'}
      puts "Could not create eligibility or update individual csr for #{person_hbx_id}"
    end
    ...
    if primary_family
      ...
      if active_thh && deter.present? && active_thh.effective_starting_on.to_date == effective_date && deter.max_aptc.to_f == aptc.to_f
        ...
        person_fm_id = primary_family.family_members.active.where(person_id: person.id).first.id
        ...
        puts "Updated individual csr for member with person hbx_id #{person.hbx_id} as this household already has one under the family for primary #{primary_person.hbx_id}"
      else
        ...
        puts "Created Eligibility for family with primary_person_hbx_id: #{primary_person.hbx_id} and given csr for all members with #{csr}"
      end
    end
