...
report_file_name = "#{Rails.root}/monthly_eligibility_report_#{TimeKeeper.date_of_record.strftime('%m_%d_%Y')}.csv"
logger_file_name = "#{Rails.root}/monthly_eligibility_report_logger_#{TimeKeeper.date_of_record.strftime('%m_%d_%Y')}.csv"
previous_month = Time.now.getlocal.prev_month
start_on = ENV['start_on']
end_on = ENV['end_on']
start_time = start_on ? Time.parse(start_on).beginning_of_month.utc : previous_month.beginning_of_month.utc
end_time = end_on ? Time.parse(end_on).end_of_month.utc : previous_month.end_of_month.utc
  ...
  CSV.open(report_file_name, 'w', force_quotes: true) do |report_csv|
    ...
    families.inject(0) do |_dummy, family|
      ...
      if EnrollRegistry.feature_enabled?(:temporary_configuration_enable_multi_tax_household_feature)
        ...
        tax_households = family.active_household.tax_households.where("created_at" => { "$gte" => start_time, "$lte" => end_time})
      end
