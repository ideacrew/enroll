    ...
    census_employees.each do |census_employee|
      begin
        ...
        if bga.present?
          ...
          puts "assigned benefit_group for #{census_employee.full_name} of ER: #{organization.legal_name}"
          count += 1
        else
          ...
          puts "New benefit_group_assignment assigned to census_employee #{census_employee.full_name} of ER: #{organization.legal_name}"
          count += 1
        end
      rescue Exception => e
        puts "#{e}"
...
puts "fixed issue for #{count} census employees"
