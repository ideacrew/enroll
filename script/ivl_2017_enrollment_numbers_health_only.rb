...
health_only_enrollments = selected_dispositions.values.reject { |sd| sd.coverage_kinds.include?("dental") }.map(&:enrollments).flatten

enroll_pol_ids = health_only_enrollments.map(&:enrollment_id)
all_ivl_count = health_only_enrollments.length
puts "2017 Health Only Enrollment Count: #{all_ivl_count}"
