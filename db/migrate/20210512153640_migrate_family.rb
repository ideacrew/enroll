  ...
  class << self
    ...
    def define_etl_variables(source)
      @extract_klass, @transform_klass = case source
                                         ...
                                         when 'atp'
                                           [::AcaEntities::Serializers::Xml::Medicaid::Atp::AccountTransferRequest, ::AcaEntities::Atp::Transformers::Cv::Family]
                                         end
    end
    ...
    def load_data(payload = {})
      ...
      # build_iap(result_payload['magi_medicaid_applications'].first.merge!(family_id: @family.id, benchmark_product_id: BSON::ObjectId.new,
                                                                        # years_to_renew: 5))
      print "."
    rescue StandardError => e
