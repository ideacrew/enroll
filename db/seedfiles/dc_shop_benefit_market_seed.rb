...
Organization.where(carrier_profile: {"$ne" => nil}).each do |org|
  ...
  if Plan.where(:carrier_profile_id => carrier_profile.id, :market => "shop", :coverage_kind => "health", :active_year => benefit_market_catalog.application_period.first.year).any?
