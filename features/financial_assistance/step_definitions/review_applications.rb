...
Given(/^that a family has a Financial Assistance application in the (.*?) state$/) do |state|
  ...
  @applications = [FactoryBot.create(:financial_assistance_application, :with_applicants, family_id: @family.id, aasm_state: state)]
...
Then(/^the user will navigate to the Review Application page$/) do
  if EnrollRegistry.feature_enabled?(:financial_assistance) && FinancialAssistanceRegistry.feature_enabled?(:display_medicaid_question)
