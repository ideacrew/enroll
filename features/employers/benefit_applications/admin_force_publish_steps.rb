...
Then(/^a warning message will appear$/) do
  expect(page.driver.browser.modal_message).to have_content('If you submit this application as is, the small business application may be ineligible for coverage through the Health Connector because it does not meet the eligibility reason(s) identified below')
end
