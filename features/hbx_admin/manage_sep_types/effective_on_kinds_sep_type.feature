Feature: User while terminating active enrollment should able to see and pick termination on kinds dates to terminate

  Scenario: Employee should able to purchase through covid QLE using first_of_this_month effective date and terminates the active enrollment with termination on kinds date 
    ...
    When Patrick Doe clicks on Shop For Plans button
    And Patrick Doe clicked on continue for plan shopping
    Then Patrick Doe should see page with SelectPlanToTerminate button
    When Patrick Doe clicks SelectPlanToTerminate button
    And Patrick Doe selects Termination on kinds date on the dropdown
