...
Then(/^Employee should see the matched employee record form$/) do
  @browser.label(text: /Acme Inc\./).wait_until_present
  screenshot("employer_search_results")
  expect(@browser.label(text: /Acme Inc\./).visible?).to be_truthy
end
...
When(/^.+ accepts? the matched employer$/) do
  scroll_then_click(@browser.label(text: /Enroll as an employee of/i))
