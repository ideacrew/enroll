...
When(/^.+ enters? the dependent info of Sorens daughter$/) do
  ...
  @browser.text_field(name: 'dependent[addresses][address_1]').set('623a Spalding Ct')
  @browser.text_field(name: 'dependent[addresses][city]').set('Washington')
  input_field = @browser.select(name: /state/).divs(xpath: "ancestor::div")[-2]
  input_field.click
  input_field.li(text: /DC/).click
  @browser.text_field(name: 'dependent[addresses][zip]').set('20001')
