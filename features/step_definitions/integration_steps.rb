...
Given(/^Hbx Admin exists$/) do
  p_staff=Permission.create(name: 'hbx_staff', modify_family: true, modify_employer: true, revert_application: true,
                            ...
                            can_access_accept_reject_identity_documents: true, can_edit_aptc: true)
...
Then(/^they should see the bot button$/) do
  bot_button = page.all('button').detect { |button| button[:id] == 'bot-button'}
  expect(bot_button.present?).to eq(true)
end
