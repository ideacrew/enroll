...
Then(/^.+ should see broker registration successful message$/) do
  expect(page).to have_content('Complete the following requirements to become a DC Health Link Registered Broker') if is_dc_or_me_site?
...
Then(/broker (.*?) should receive application (.*?) notification$/) do |broker_name, notification_kind|
  ...
  subject =
    ...
    when 'extended'
      'Action Needed - Complete Broker Training for #{Settings.site.short_name} for Business'
    end
  open_email(
    broker_email_address,
    :with_subject => subject
