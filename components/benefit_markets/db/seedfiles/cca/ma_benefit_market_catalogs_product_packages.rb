Mongoid::Migration.say_with_time("Load MA Benefit Market Catalogs") do
  ...
  site = BenefitSponsors::Site.where(site_key: "#{SEnrollRegistry[:enroll_app].setting(:subdomain).item}").first
  ...
  [2018, 2019].each do |calender_year|
    ...
    benefit_market_catalog = benefit_market.benefit_market_catalogs.create!({
      title: "#{EnrollRegistry[:enroll_app].setting(:state_abbreviation).item} #{EnrollRegistry[:enroll_app].setting(:short_name).item} SHOP Benefit Catalog",
