  ...
  module Datatables
    class BrokerAgencyPlanDesignOrganizationDatatable < ::Effective::MongoidDatatable
        ...
        table_column :actions, :width => '50px', :proc => Proc.new { |row|
          dropdown = [
           # Link Structure: ['Link Name', link_path(:params), 'link_type'], link_type can be 'ajax', 'static', or 'disabled'
            ...
            ['Show/Add Roles', benefit_sponsors.profiles_employers_employer_staff_roles_path(profile_id: row.sponsor_profile_id, employer_actions_id: "employer_actions_#{row.sponsor_profile_id}"), 'ajax'],
      ...
      def add_roles_link_type(allow)
        ...
        policy_accepted_and_allow ? 'ajax' : 'disabled'
      end
