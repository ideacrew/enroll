module Notifier
  class Builders::EmployerProfile
    ...
    def total_amount_due
      merge_model.total_amount_due = number_to_currency(active_benefit_sponsorship.benefit_applications.where(:"aasm_state".in => ["enrollment_closed", "enrollment_eligible", "enrollment_open"]).first.enrollments_till_next_month(invoice_month.to_date).map(&:total_premium).sum)
    end
