    ...
    class BenefitSponsorsGeneralAgencyFamilyDataTable < Effective::MongoidDatatable
      ...
      datatable do
        table_column :name, :label => 'Name', :proc => Proc.new { |row|
          ...
          link_to_with_noopener_noreferrer(pp.full_name, main_app.resume_enrollment_exchanges_agents_path(person_id: pp.id))
      ...
      def authorized?(current_user, _controller, _action, _resource)
        ...
        organizations = BenefitSponsors::Organizations::Organization.where(:"profiles._id" => attributes[:id])
