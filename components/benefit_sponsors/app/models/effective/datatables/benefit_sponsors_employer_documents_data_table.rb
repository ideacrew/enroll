    ...
    class BenefitSponsorsEmployerDocumentsDataTable < Effective::MongoidDatatable
      datatable do
        ...
        table_column :name, :label => 'Doc Name', :proc => Proc.new { |row|
          link_to raw('<i class="far fa-file-alt pull-left mr-1"></i> ') + h(row.title), "", 'target' => "iframe_#{row.id}", 'data-target' => "#employeeModal_#{row.id}", "data-toggle" => "modal", "class" => "word-break-attestation"
        }, :filter => false, :sortable => false
