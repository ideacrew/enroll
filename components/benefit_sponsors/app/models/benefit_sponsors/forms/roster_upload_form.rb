    ...
    class RosterUploadForm
      ...
      def roster_template
        ...
        unless (template_date == TEMPLATE_DATE && template_version == TEMPLATE_VERSION && header_valid?(sheet.row(2)))
          self.errors.add(:base, "Unrecognized Employee Census spreadsheet format. Contact #{BenefitSponsorsRegistry[:enroll_app].settings(:short_name).item} for current template.")
