  ...
  module Accounts
    class GeneralAgencyAccount
      ...
      def general_agency_profile=(profile)
        raise StandardError, "expected GeneralAgencyProfile" unless profile.class.to_s.match(/GeneralAgencyProfile/)
      ...
      def general_agency_profile
        ...
        @general_agency_profile =
          if benefit_sponsrship_general_agency_profile_id.blank?
            GeneralAgencyProfile.find(general_agency_profile_id)
          else
      ...
      def broker_agency_profile=(profile)
        raise StandardError, "expected BrokerAgencyProfile" unless profile.class.to_s.match(/BrokerAgencyProfile/)
      ...
      def broker_agency_profile
        ...
        @broker_agency_profile =
          if benefit_sponsrship_broker_agency_profile_id.blank?
            BrokerAgencyProfile.find(broker_agency_profile_id)
          else
      ...
      def ga_name
        Rails.cache.fetch("general-agency-name-#{general_agency_profile_id}", expires_in: 12.hours) do
          legal_name
        end
      end
      ...
      def for_broker_agency_account?(ba_account)
        ...
        return((start_on >= ba_account.start_on) && (start_on <= ba_account.end_on)) if ba_account.end_on.present?
      ...
      class << self
        def find(id)
          ...
          pdo.general_agency_accounts.where(:_id => BSON::ObjectId.from_string(id)).first if pdo.present?
        end
      end
    end
  end
