      ...
      describe "given:
        ...
        context 'for invalid relationships' do
          ...
          let(:invalid_family_group_enrollment) do
            BenefitSponsors::Enrollments::GroupEnrollment.new(
              member_enrollments: [employee_enrollment, spouse_member_enrollment, parent_enrollment],
              rate_schedule_date: rate_schedule_date,
              coverage_start_on: coverage_start_date,
              previous_product: nil,
              product: product,
          ...
          let(:calculator_state) do
            ...
            cal_klass = BenefitSponsors::PricingCalculators::ShopSimpleListBillPricingCalculator::CalculatorState
            cal_klass.new(age_calculator,
                          'reference_product',
                          pricing_model,
                          'pricing_unit_map',
                          roster_coverage,
          ...
          it 'should raise UnmatchedRelationshipError' do
            error_klass = ::BenefitSponsors::PricingCalculators::UnmatchedRelationshipError
            expect{calculator_state.add(parent)}.to raise_error(error_klass)
          end

          it 'calculates the total contribution' do
            calculation_result = pricing_calculator.calculate_price_for(
              pricing_model,
