...
RSpec.describe 'BenefitSponsors::ModelEvents::RenewalEmployerOpenEnrollmentCompleted', dbclean: :after_each do
  ...
  let!(:date_mock_object) { double("Date", day: (model_instance.open_enrollment_period.max + 1.day).day)}
  ...
  describe "NoticeBuilder" do
    ...
    context "when notice event received" do
      ...
      it "should return publish due date" do
        due_date = Date.new(model_instance.start_on.prev_month.year, model_instance.start_on.prev_month.month, Settings.aca.shop_market.renewal_application.publish_due_day_of_month).strftime('%m/%d/%Y')
