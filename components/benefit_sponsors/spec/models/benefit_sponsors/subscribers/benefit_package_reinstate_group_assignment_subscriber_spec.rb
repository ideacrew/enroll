...
RSpec.describe BenefitSponsors::Subscribers::BenefitPackageReinstateGroupAssignmentSubscriber, :dbclean => :after_each do
  ...
  let(:subscriber) do
    BenefitSponsors::Subscribers::BenefitPackageReinstateGroupAssignmentSubscriber.new
  end
  ...
  describe "that has an exception while reinstating the assignment" do
    before :each do
      ...
      allow_any_instance_of(BenefitSponsors::BenefitPackages::BenefitPackage).to receive(:reinstate_benefit_group_assignment).with(benefit_group_assignment).and_raise(error)
      allow(subscriber).to receive(:notify).with(
