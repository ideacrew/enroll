...
RSpec.describe BenefitSponsors::Validators::BenefitSponsorships::BenefitSponsorshipContract do
  ...
  let(:error_message1)                 { {:market_kind => ["is missing", "must be Symbol"], :organization_id => ["is missing", "must be BSON::ObjectId"]} }
  let(:error_message2)                 { {:organization_id => ["is missing", "must be BSON::ObjectId"], :registered_on => ["must be a date"]} }
