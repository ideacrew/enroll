function stopEditingBenefit() {
  $('.driver-question, .instruction-row, .benefit-kind').removeClass(
    'disabled'
  );
...
function startEditingBenefit(benefit_kind) {
  $(
    '.driver-question, .instruction-row, .benefit-kind:not(#' +
      benefit_kind +
      ')'
  ).addClass('disabled');
...
function startEditing(parentContainer) {
  ...
  $('.driver-question, .instruction-row, .add_new_benefit_kind').addClass(
    'disabled'
  );
  ...
  $('.driver-question input:not(input[type=submit]), .disabled a').attr(
    'disabled',
    true
  );
  $(parentContainer)
    .find('.active input:not([type=submit])')
    .removeAttr('disabled');
...
function stopEditing() {
  ...
  $(
    '.driver-question, .instruction-row, .add_new_benefit_kind',
    '.benefit'
  ).removeClass('disabled');
  $('.disabled a').attr('tabindex', 0);
  $('.disabled a').removeClass('disabled');
  $('.disabled input:not(input[type=submit]), .disabled a').removeAttr(
    'disabled'
  );
  $('.driver-question input:not(input[type=submit]), .disabled a').removeAttr(
    'disabled'
  );
  $(
    '.driver-question, .instruction-row, .disabled a, .benefits-list, #nav-buttons a, .benefit, .add_new_benefit_kind'
  ).removeClass('disabled');
}
...
function handleEsiFields(form, isEsi, isHra, isMvsq) {
  ...
  if (isEsi == 'true') {
    ...
    if (isHra == 'true') {
      ...
      form
        .querySelectorAll('.non-hra-questions input, non-hra-questions select')
      ...
      form
        .querySelectorAll('.hra-questions input, hra-questions select')
        .forEach(function (input) {
          var label = form.querySelector("label[for='" + input.id + "']");
          if (
            (label && label.classList.contains('required')) ||
    ...
    if (isMvsq == 'true') {
      ...
      form
        .querySelectorAll('.mvsq-questions input, mvsq-questions select')
        .forEach(function (input) {
          var label = form.querySelector("label[for='" + input.id + "']");
          if (
            (label && label.classList.contains('required')) ||
...
document.addEventListener('turbolinks:load', function () {
  ...
  $('.benefit-kinds').on(
    ...
    function (event) {
      ...
      var newBenefitFormEl = document.getElementById(
        'new-benefit-form-' + kind
      );
  ...
  $('.benefit-kinds').on(
    ...
    function (event) {
      ...
      var benefitList = $(button)
        .parents('.benefit-kinds')
        .find('.benefits-list');
      ...
      if ($(benefitList).find('.benefit').length > 0) {
        ...
        $(
          '#has_' +
            (kind == 'is_enrolled' ? 'enrolled' : 'eligible') +
            '_health_coverage_true'
        )
          .prop('checked', false)
          .trigger('change');
      }
  ...
  $('.benefit-kinds').on(
    ...
    function (event) {
      ...
      if (benefitList.querySelectorAll('.benefit').length == 0) {
        document
          .getElementById('new-benefit-form-' + kind)
          .classList.remove('hidden');
      ...
      benefitList
        .querySelectorAll('.benefit.active')
  ...
  if ($('.benefit-kinds').length) {
    $(window).bind('beforeunload', function (e) {
      if (
        !currentlyEditing() ||
        $('#unsavedBenefitChangesWarning:visible').length
      )
        return undefined;

      (e || window.event).returnValue =
        'You have an unsaved benefit, are you sure you want to proceed?'; //Gecko + IE
      return 'You have an unsaved benefit, are you sure you want to proceed?';
    });

    $(document).on(
      'click',
      'a[href]:not(.disabled):not(.benefit-support-modal):not([target="_blank"])',
      function (e) {
        if (currentlyEditing()) {
          e.preventDefault();
          var self = this;
    ...
    $(document).on(
      'click',
      '.benefit-kind input[type="checkbox"]',
      function (e) {
        var value = e.target.checked,
          self = this;
        if (value) {
          var newBenefitFormEl = $(this)
              .parents('.benefit-kind')
              .children('.new-benefit-form'),
            benefitListEl = $(this)
              .parents('.benefit-kind')
              .find('.benefits-list');
          if (newBenefitFormEl.find('select').data('selectric'))
            newBenefitFormEl.find('select').selectric('destroy');
          var clonedForm = newBenefitFormEl
            .clone(true, true)
            .removeClass('hidden')
          ...
          $('#destroyAllBenefits .modal-continue-button').click(function (e) {
            ...
            $(self)
              .parents('.benefit-kind')
              .find('.benefits-list > .benefit')
              .each(function (i, benefit) {
                var url = $(benefit)
                  .attr('id')
                  .replace('benefit_', 'benefits/');
                $(benefit).remove();
    ...
    $('body').on('change', '#has_enrolled_health_coverage_true', function () {
      if ($('#has_enrolled_health_coverage_true').is(':checked')) {
        ...
        if ($('#enrolled-benefit-kinds .benefit').length > 0) {
          ...
          if ($('body').data('bs4')) {
            startEditing(
              $('#has_enrolled_health_coverage_true').closest(
                '.driver-question'
              )
            );
          }
    ...
    $('body').on('change', '#has_eligible_health_coverage_true', function () {
      if ($('#has_eligible_health_coverage_true').is(':checked')) {
        ...
        if ($('#eligible-benefit-kinds .benefit').length > 0) {
          ...
          if ($('body').data('bs4')) {
            startEditing(
              $('#has_eligible_health_coverage_true').closest(
                '.driver-question'
              )
            );
          }
    ...
    $('#has_eligible_medicaid_cubcare_true').on('change', function (e) {
      var attributes = {};
      $(
        '#has_eligibility_changed_true, #has_eligibility_changed_false, #has_household_income_changed_true, #has_household_income_changed_false, #person_coverage_end_on'
      ).each(function (i, ele) {
    ...
    $('#has_dependent_with_coverage_true').on('change', function (e) {
      var attributes = {};
      $(
        '#has_dependent_with_coverage_true, #has_dependent_with_coverage_false, #dependent_job_end_on'
      ).each(function (i, ele) {
        attributes[$(this).attr('name')] = ' ';
        $(this).val('');
      });
