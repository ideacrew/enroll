    ...
    class Applicant
      ...
      def validate_tribe_details
        if EnrollRegistry[:indian_alaskan_tribe_details].enabled?
          ...
          if FinancialAssistanceRegistry[:featured_tribes_selection].enabled?
            self.errors.add(:tribal_name, "is required when native american / alaska native is selected") if !tribal_name.present? && tribe_codes.include?("OT") && tribal_state = "ME"
          else
            self.errors.add(:tribal_name, "is required when native american / alaska native is selected") if !tribal_name.present?
          end
        else
