...
RSpec.describe ::FinancialAssistance::Operations::Applications::AptcCsrCreditEligibilities::Renewals::PublishRenewalRequest, dbclean: :after_each do
  ...
  context 'When connection is available' do
    context 'determination_submission.requested' do
      before do
        ...
        @result = subject.call({ payload: application_params.success, event_name: 'determination_submission.requested' })
      end
    ...
    context 'renewal.requested' do
      before do
        params = { payload: { family_id: application.family_id, renewal_year: application.assistance_year.next }, event_name: 'renewal.requested' }
    ...
    context 'renewed' do
      before do
        ...
        params = { payload: { application_hbx_id: application.hbx_id }, event_name: 'renewed' }
        @result = subject.call(params)
      end
  ...
  context 'failure' do
    context 'missing keys' do
      context 'missing payload' do
        before do
          @result = subject.call({ event_name: ['renewal.requested', 'determination_added'].sample })
        end
    ...
    context 'missing values or invalid values' do
      context 'missing value for payload' do
        before do
          @result = subject.call({ payload: nil, event_name: ['renewal.requested', 'determination_added'].sample })
        end
      ...
      context 'invalid value for payload' do
        before do
          @result = subject.call({ payload: { test: 'test' }.to_s, event_name: ['renewal.requested', 'determination_added'].sample })
        end
      ...
      context 'invalid value for event_name' do
        ...
        it 'should return failure with error message' do
          expect(@result.failure).to match(/Invalid event_name: test_event for key event_name, must be one of/)
        end
      end
    end
  end
