...
if ExchangeTestingConfigurationHelper.individual_market_is_enabled?
  RSpec.describe FinancialAssistance::BenefitsController, dbclean: :after_each, type: :controller do
    ...
    let(:benefit) do
      primary_applicant.benefits.create(
        {
    ...
    describe '#step' do
      ...
      context 'logged in user has developer role' do
        it 'denies access and redirects to a different path' do
          ...
          expect(flash[:error]).to eq(
            'Access not allowed for financial_assistance/benefit_policy.step?, (Pundit policy)'
          )
        end
      end
    end
    ...
    describe '#update' do
      ...
      context 'logged in user has developer role' do
        it 'denies access and redirects to a different path' do
          ...
          expect(flash[:error]).to eq(
            'Access not allowed for financial_assistance/benefit_policy.update?, (Pundit policy)'
          )
        end
      end
    end
    ...
    describe '#destroy' do
      ...
      context 'logged in user has developer role' do
        it 'denies access and redirects to a different path' do
          ...
          expect(flash[:error]).to eq(
            'Access not allowed for financial_assistance/benefit_policy.destroy?, (Pundit policy)'
          )
        end
      end
    end
