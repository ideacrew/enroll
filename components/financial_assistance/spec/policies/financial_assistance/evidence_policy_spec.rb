...
if ExchangeTestingConfigurationHelper.individual_market_is_enabled?
  RSpec.describe FinancialAssistance::EvidencePolicy, dbclean: :after_each, type: :model do
    ...
    let(:evidence) do
      primary_applicant.create_income_evidence(
        key: :income,
        title: 'Income',
        aasm_state: 'pending',
        due_on: nil,
      ...
      context 'when a valid user is logged in' do
        context 'when the user is a consumer' do
          ...
          context 'with ridp verified' do
            it 'grants access' do
              expect(subject).to permit(logged_in_user, evidence)
            end
          end
    ...
    permissions :update_evidence? do
      context 'when a valid user is logged in' do
        context 'when the user is a consumer' do
          let(:user_of_family) { FactoryBot.create(:user, person: person) }
          let(:logged_in_user) { user_of_family }
    ...
    permissions :extend_due_date? do
      context 'when a valid user is logged in' do
        context 'when the user is a consumer' do
          let(:user_of_family) { FactoryBot.create(:user, person: person) }
          let(:logged_in_user) { user_of_family }
