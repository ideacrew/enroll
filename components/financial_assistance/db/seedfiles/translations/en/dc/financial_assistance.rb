...
FINANCIAL_ASSISTANCE_TRANSLATIONS = {
  ...
  "en.faa.question.private_individual_and_family_coverage" => "#{EnrollRegistry[:enroll_app].setting(:short_name).item} Individual & Family coverage",
  ...
  "en.faa.question.coverage_obtained_through_another_exchange" => "Coverage obtained through a non-#{EnrollRegistry[:enroll_app].setting(:short_name).item} marketplace",
  ...
  "en.faa.renewal_process_1" => "This process is called renewal. #{EnrollRegistry[:enroll_app].setting(:short_name).item} will review eligibility by checking its records and other electronic data sources",
  "en.faa.renewal_process_2" => " including, with my consent, information about my federal tax returns from the IRS.",
  "en.faa.send_notice_1" => "#{EnrollRegistry[:enroll_app].setting(:short_name).item} will send me a notice that includes the information it has found by checking its records and other electronic data sources,",
  ...
  "en.faa.report_changes_1" => "I can report changes by going online and logging into 'My Account', by calling #{EnrollRegistry[:enroll_app].setting(:short_name).item}'s Contact Center toll free at '#{Settings.contact_center.phone_number}' TTY: 711, ",
