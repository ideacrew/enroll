...
#      execute "rm -rf #{shared_path}/public/assets/*"
      within release_path do
        with rails_env: fetch(:rails_env) do
          ...
          client_variable = ENV['CLIENT'].downcase || ENV['client'].downcase || EnrollRegistry[:enroll_app]&.setting(:site_key)&.item.to_s
          puts("Switching to #{client_variable} configuration.") unless client_variable.blank?
          execute :rake, "configuration:client_configuration_toggler client='#{client_variable}'" unless client_variable.blank?
          puts("No client configuration present, using current committed configuration.") if client_variable.blank?
        end
      end
    end
  end
